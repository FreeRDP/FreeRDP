...
static int mac_shadow_capture_get_dirty_region(macShadowSubsystem* subsystem)
{
	...
	for (index = 0; index < numRects; index++)
	{
		...
		region16_union_rect(&(subsystem->invalidRegion), &(subsystem->invalidRegion),
		                    &invalidRect);
	}
              ...
              IOSurfaceRef frameSurface, CGDisplayStreamUpdateRef updateRef)
{
	...
	region16_intersect_rect(&(subsystem->invalidRegion),
	                        &(subsystem->invalidRegion), &surfaceRect);
	...
	if (!region16_is_empty(&(subsystem->invalidRegion)))
	{
		...
		if (subsystem->retina)
		{
			freerdp_image_copy_from_retina(surface->data, PIXEL_FORMAT_XRGB32,
			                               surface->scanline,
        ...
        wMessage* message)
{
	switch (message->id)
	{
		case SHADOW_MSG_IN_REFRESH_OUTPUT_ID:
			{
				UINT32 index;
				SHADOW_MSG_IN_REFRESH_OUTPUT* msg = (SHADOW_MSG_IN_REFRESH_OUTPUT*)
				                                    message->wParam;
		...
		case SHADOW_MSG_IN_SUPPRESS_OUTPUT_ID:
			{
				SHADOW_MSG_IN_SUPPRESS_OUTPUT* msg = (SHADOW_MSG_IN_SUPPRESS_OUTPUT*)
				                                     message->wParam;
