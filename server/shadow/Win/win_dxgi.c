                                     ...
                                     BYTE** ppDstData, int* pnDstStep, int x, int y, int width, int height)
{
	...
	subsystem->dxgiDeviceContext->lpVtbl->CopySubresourceRegion(subsystem->dxgiDeviceContext,
	        (ID3D11Resource*) subsystem->dxgiStage, 0, 0, 0, 0, (ID3D11Resource*) subsystem->dxgiDesktopImage,
	        0, &Box);
...
int win_shadow_dxgi_get_invalid_region(winShadowSubsystem* subsystem)
{
	...
	{
		WLog_ERR(TAG,
		         "IDXGIOutputDuplication::GetFrameMoveRects failure: %s (0x%08lX) Size: %u Total %u Used: %u",
	...
	{
		WLog_ERR(TAG,
		         "IDXGIOutputDuplication::GetFrameDirtyRects failure: %s (0x%08lX) Size: %u Total %u Used: %u",
