                                            ...
                                            BOOL toBack)
{
	...
	if (toBack)
	{
		...
		ev.channel_id = WINPR_SAFE_INT_CAST(UINT16, channel->front_channel_id);
	...
	r = ps->context.peer->SendChannelPacket(
	    ps->context.peer, WINPR_SAFE_INT_CAST(UINT16, channel->front_channel_id), currentPacketSize,
	    flags, Stream_Buffer(currentPacket), Stream_GetPosition(currentPacket));
                                                    ...
                                                    size_t totalSize)
{
	...
	switch (channel->channelMode)
	{
		case PF_UTILS_CHANNEL_PASSTHROUGH:
			ev.channel_id = WINPR_SAFE_INT_CAST(UINT16, channel->back_channel_id);
			ev.channel_name = channel->channel_name;
			ev.data = xdata;
			ev.data_len = xsize;
			ev.flags = flags;
