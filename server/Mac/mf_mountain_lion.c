...
int mf_mlion_screen_updates_init()
{
	...
	opts = CFDictionaryCreate(kCFAllocatorDefault, (const void**) keys, (const void**) values, 1, NULL,
	                          NULL);
...
int mf_mlion_peek_dirty_region(RFX_RECT* invalid)
{
	size_t num_rects, i;
	CGRect dirtyRegion;
	const CGRect* rects = CGDisplayStreamUpdateGetRects(lastUpdate, kCGDisplayStreamUpdateDirtyRects,
	                      &num_rects);
