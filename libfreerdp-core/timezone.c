...
void rdp_get_client_time_zone(STREAM* s, rdpSettings* settings)
{
	...
	tz = rdp_detect_windows_time_zone(clientTimeZone->bias);
	...
	if (tz != NULL)
	{
		...
		if ((tz->SupportsDST) && (tz->RuleTableCount > 0))
		{
			...
			rule = rdp_get_current_time_zone_rule(tz->RuleTable, tz->RuleTableCount);
...
boolean rdp_read_client_time_zone(STREAM* s, rdpSettings* settings)
{
	...
	str = freerdp_uniconv_in(settings->uniconv, stream_get_tail(s), 64 / 2);
	stream_seek(s, 64);
...
{
	size_t length;
	uint8* standardName = NULL;
	uint8* daylightName = NULL;
