...
# the RHEL-a-likes don't have the SDL3 sub-packages, so force them to SDL2
%if 0%{?suse_version} >= 1600 || 0%{?fedora} > 41
...
# Note: the compound-boolean statements DO NOT WORK on OpenSuse
# (Open)Suse
...
# Tumbleweed renamed this package, no idea why
...
%if 0%{?fedora} || 0%{?rhel} > 9 || 0%{?suse_version} >= 1600
BuildRequires: pcsc-lite-devel
...
%if 0%{?suse_version} >= 1600
    -DCMAKE_TOOLCHAIN_FILE=$PWD/packaging/rpm/suse-sanitizer-toolchain.cmake \
%endif
...
%if 0%{?fedora} || 0%{?rhel} > 8
    -DWITH_FDK_AAC=ON \
%endif
%if 0%{?fedora} || 0%{?rhel} > 8 || 0%{?suse_version} >= 1600
    -DWITH_FFMPEG=ON \
    -DWITH_DSP_FFMPEG=ON \
%else
    -DWITH_FFMPEG=OFF \
...
%if %{defined suse_version}
# SUSE's CMake configure macro leaves the shell in build/.
cd ..
%endif
...
%changelog
* Mon May 18 2026 FreeRDP Team <team@freerdp.com> - 3.26.1-1
- Changes in specfile to cover actual testing on SuSE and RHEL
