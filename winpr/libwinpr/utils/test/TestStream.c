	...
	for (size_t offset = 1; offset <= 2; offset++)
	{
		for (size_t fill = 0; fill <= 1; fill++)
		{
			...
			if ((rc != (SSIZE_T)length) || (Stream_GetPosition(s) != end))
				goto out;
			Stream_Write(s, channel, strlen(channel) + 1);
			...
			if (fill && ((Stream_Buffer(s)[end - 2] != 0) || (Stream_Buffer(s)[end - 1] != 0)))
				goto out;
			if (strcmp((const char*)Stream_Buffer(s) + end, channel) != 0)
				goto out;
		}
	}

	/* Reject truncation instead of silently writing a partial string. */
	Stream_ResetPosition(s);
	if (Stream_Write_UTF16_String_From_UTF8(s, 2, name, length, TRUE) >= 0)
		goto out;
