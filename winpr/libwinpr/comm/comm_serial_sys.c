...
static BOOL _get_properties(WINPR_COMM* pComm, COMMPROP* pProperties)
{
	...
	pProperties->dwSettableParams = SP_BAUD | SP_DATABITS | SP_HANDSHAKING | SP_PARITY | SP_PARITY_CHECK
	                                | /*SP_RLSD |*/ SP_STOPBITS;
	...
	pProperties->wSettableData = DATABITS_5 | DATABITS_6 | DATABITS_7 |
...
static BOOL _set_handflow(WINPR_COMM* pComm, const SERIAL_HANDFLOW* pHandflow)
{
	     ...
	     !(pHandflow->FlowReplace & SERIAL_RTS_CONTROL)))
	{
		CommLog_Print(WLOG_WARN,
		              "SERIAL_DTR_CONTROL:%s and SERIAL_RTS_CONTROL:%s cannot be different, HUPCL will be set since it is claimed for one of the both lines.",
	...
	{
		CommLog_Print(WLOG_WARN,
		              "SERIAL_CTS_HANDSHAKE:%s and SERIAL_RTS_HANDSHAKE:%s cannot be different, CRTSCTS will be set since it is claimed for one of the both lines.",
...
static BOOL _set_lines(WINPR_COMM* pComm, UINT32 lines)
{
	...
	{
		CommLog_Print(WLOG_WARN, "TIOCMBIS ioctl failed, lines=0x%"PRIX32", errno=[%d] %s", lines, errno,
		              strerror(errno));
		SetLastError(ERROR_IO_DEVICE);
		return FALSE;
	}
...
static BOOL _clear_lines(WINPR_COMM* pComm, UINT32 lines)
{
	...
	{
		CommLog_Print(WLOG_WARN, "TIOCMBIC ioctl failed, lines=0x%"PRIX32", errno=[%d] %s", lines, errno,
		              strerror(errno));
		SetLastError(ERROR_IO_DEVICE);
		return FALSE;
	}
...
static BOOL _set_wait_mask(WINPR_COMM* pComm, const ULONG* pWaitMask)
{
	...
	{
		CommLog_Print(WLOG_WARN,
		              "Not all wait events supported (Serial.sys), requested events= 0x%08"PRIX32", possible events= 0x%08"PRIX32"",
...
static BOOL _set_queue_size(WINPR_COMM* pComm, const SERIAL_QUEUE_SIZE* pQueueSize)
{
	...
	if (pQueueSize->InSize > N_TTY_BUF_SIZE)
		CommLog_Print(WLOG_WARN,
		              "Requested an incompatible input buffer size: %"PRIu32", keeping on with a %"PRIu32" bytes buffer.",
		              pQueueSize->InSize, N_TTY_BUF_SIZE);

	if (pQueueSize->OutSize > N_TTY_BUF_SIZE)
		CommLog_Print(WLOG_WARN,
		              "Requested an incompatible output buffer size: %"PRIu32", keeping on with a %"PRIu32" bytes buffer.",
