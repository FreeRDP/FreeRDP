 ...
 */
static const speed_t s_BAUD_TABLE[][3] = {
#ifdef B0
	{ B0, 0, 0 }, /* hang up */
#endif
#ifdef B50
...
static BOOL s_get_properties(WINPR_COMM* pComm, COMMPROP* pProperties)
{
	...
	for (i = 0; s_BAUD_TABLE[i][0] < BAUD_TABLE_END; i++)
	{
		pProperties->dwSettableBaud |= s_BAUD_TABLE[i][2];
	}
...
static BOOL s_set_baud_rate(WINPR_COMM* pComm, const SERIAL_BAUD_RATE* pBaudRate)
{
	...
	for (i = 0; s_BAUD_TABLE[i][0] < BAUD_TABLE_END; i++)
	{
		if (s_BAUD_TABLE[i][1] == pBaudRate->BaudRate)
		{
			newSpeed = s_BAUD_TABLE[i][0];
...
static BOOL s_get_baud_rate(WINPR_COMM* pComm, SERIAL_BAUD_RATE* pBaudRate)
{
	...
	for (i = 0; s_BAUD_TABLE[i][0] < BAUD_TABLE_END; i++)
	{
		if (s_BAUD_TABLE[i][0] == currentSpeed)
		{
			pBaudRate->BaudRate = s_BAUD_TABLE[i][1];
			return TRUE;
		}
	}
...
static const ULONG s_SERIAL_SYS_SUPPORTED_EV_MASK =
...
static BOOL s_set_wait_mask(WINPR_COMM* pComm, const ULONG* pWaitMask)
{
	...
	possibleMask = *pWaitMask & s_SERIAL_SYS_SUPPORTED_EV_MASK;
...
static SERIAL_DRIVER s_SerialSys = {
