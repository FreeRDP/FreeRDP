...
static BOOL test_generic(HANDLE hComm)
{
	...
	{
		fprintf(stderr,
		        "SetCommTimeouts succeeded with ReadIntervalTimeout and ReadTotalTimeoutConstant set to MAXULONG. GetLastError: 0x%08x\n",
		        GetLastError());
		return FALSE;
	}
	...
	{
		fprintf(stderr,
		        "SetCommTimeouts failure, expected GetLastError to return ERROR_INVALID_PARAMETER and got: 0x%08x\n",
		        GetLastError());
		return FALSE;
	}
