...
WinPrNdrContext* winpr_ndr_context_new(BOOL bigEndianDrep, BYTE version)
{
	WinPrNdrContext* ret = calloc(1, sizeof(*ret));
	if (!ret)
		return NULL;
...
WinPrNdrContext* winpr_ndr_context_copy(const WinPrNdrContext* src)
{
	WINPR_ASSERT(src);
...
void winpr_ndr_context_free(WinPrNdrContext* context)
{
	if (context)
	{
...
static void winpr_ndr_context_bytes_read(WinPrNdrContext* context, size_t len)
{
	WINPR_ASSERT(context);
	context->indentLevels[context->currentLevel] += len;
}

static void winpr_ndr_context_bytes_written(WinPrNdrContext* context, size_t len)
{
	winpr_ndr_context_bytes_read(context, len);
}

WinPrNdrContext* winpr_ndr_read_header(wStream* s)
{
	...
	return winpr_ndr_context_new((drep != 0x10), version);
}

BOOL winpr_ndr_write_header(WinPrNdrContext* context, wStream* s)
{
	WINPR_ASSERT(context);
...
BOOL winpr_ndr_skip_bytes(WinPrNdrContext* context, wStream* s, size_t nbytes)
{
	WINPR_ASSERT(context);
...
BOOL winpr_ndr_read_align(WinPrNdrContext* context, wStream* s, size_t sz)
{
	WINPR_ASSERT(context);
...
BOOL winpr_ndr_write_align(WinPrNdrContext* context, wStream* s, size_t sz)
{
	WINPR_ASSERT(context);
...
BOOL winpr_ndr_read_pickle(WinPrNdrContext* context, wStream* s)
{
	WINPR_ASSERT(context);
	...
	/* NDR format label */
	if (!winpr_ndr_read_uint32(context, s, &v) || v != 0x20000)
		return FALSE;

	return winpr_ndr_read_uint32(context, s, &v); // padding
}

BOOL winpr_ndr_write_pickle(WinPrNdrContext* context, wStream* s)
{
	WINPR_ASSERT(context);

	/* NDR format label */
	if (!winpr_ndr_write_uint32(context, s, 0x20000))
		return FALSE;

	winpr_ndr_write_uint32(context, s, 0); /* padding */
	return TRUE;
}

BOOL winpr_ndr_read_constructed(WinPrNdrContext* context, wStream* s, wStream* target)
{
	WINPR_ASSERT(context);
	...
	/* len */
	if (!winpr_ndr_read_uint32(context, s, &len))
		return FALSE;

	/* padding */
	if (!winpr_ndr_skip_bytes(context, s, 4))
		return FALSE;
...
BOOL winpr_ndr_start_constructed(WinPrNdrContext* context, wStream* s)
{
	WINPR_ASSERT(context);
...
BOOL winpr_ndr_end_constructed(WinPrNdrContext* context, wStream* s)
{
	WINPR_ASSERT(context);
	...
	if (len > UINT32_MAX)
		return FALSE;
	if (!winpr_ndr_write_uint32(context, &staticS, (UINT32)len))
		return FALSE;
...
BOOL winpr_ndr_read_uint8(WinPrNdrContext* context, wStream* s, BYTE* v)
{
	WINPR_ASSERT(context);
	...
	winpr_ndr_context_bytes_read(context, 1);
	return TRUE;
}

BOOL winpr_ndr_read_uint8_(WinPrNdrContext* context, wStream* s, const void* hints, void* v)
{
	WINPR_UNUSED(hints);
	return winpr_ndr_read_uint8(context, s, (BYTE*)v);
}

BOOL winpr_ndr_write_uint8(WinPrNdrContext* context, wStream* s, BYTE v)
{
	if (!Stream_EnsureRemainingCapacity(s, 1))
		return FALSE;

	Stream_Write_UINT8(s, v);
	winpr_ndr_context_bytes_written(context, 1);
	return TRUE;
}

BOOL winpr_ndr_write_uint8_(WinPrNdrContext* context, wStream* s, const void* hints, const void* v)
{
	WINPR_ASSERT(context);
	WINPR_ASSERT(s);
	WINPR_ASSERT(v);
	...
	return winpr_ndr_write_uint8(context, s, *(const BYTE*)v);
}

const WinPrNdrMessageDescr winpr_ndr_uint8_descr_s = {
	WINPR_NDR_ARITY_SIMPLE, 1, winpr_ndr_read_uint8_, winpr_ndr_write_uint8_, NULL, NULL
};
...
#define SIMPLE_TYPE_IMPL(PREFIX, UPPERTYPE, LOWERTYPE)                                           \
	BOOL PREFIX##_read_##LOWERTYPE(WinPrNdrContext* context, wStream* s, UPPERTYPE* v)           \
	{                                                                                            \
		WINPR_ASSERT(context);                                                                   \
                                                                                                 \
		if (!Stream_CheckAndLogRequiredLength(TAG, s, sizeof(UPPERTYPE)))                        \
			return FALSE;                                                                        \
                                                                                                 \
		if (!winpr_ndr_read_align(context, s, sizeof(UPPERTYPE)))                                \
			return FALSE;                                                                        \
                                                                                                 \
		if (context->bigEndianDrep)                                                              \
			...
			Stream_Read_##UPPERTYPE(s, *v);                                                      \
                                                                                                 \
		winpr_ndr_context_bytes_read(context, sizeof(UPPERTYPE));                                \
		return TRUE;                                                                             \
	}                                                                                            \
                                                                                                 \
	BOOL PREFIX##_read_##LOWERTYPE##_(WinPrNdrContext* context, wStream* s, const void* hints,   \
	                                  void* v)                                                   \
	{                                                                                            \
		WINPR_UNUSED(hints);                                                                     \
		return PREFIX##_read_##LOWERTYPE(context, s, (UPPERTYPE*)v);                             \
	}                                                                                            \
                                                                                                 \
	BOOL PREFIX##_write_##LOWERTYPE(WinPrNdrContext* context, wStream* s, UPPERTYPE v)           \
	{                                                                                            \
		if (!winpr_ndr_write_align(context, s, sizeof(UPPERTYPE)) ||                             \
		...
		if (context->bigEndianDrep)                                                              \
			...
			Stream_Write_##UPPERTYPE(s, v);                                                      \
                                                                                                 \
		winpr_ndr_context_bytes_written(context, sizeof(UPPERTYPE));                             \
		return TRUE;                                                                             \
	}                                                                                            \
                                                                                                 \
	BOOL PREFIX##_write_##LOWERTYPE##_(WinPrNdrContext* context, wStream* s, const void* hints,  \
	                                   const void* v)                                            \
	{                                                                                            \
		WINPR_ASSERT(context);                                                                   \
		WINPR_ASSERT(s);                                                                         \
		...
		WINPR_UNUSED(hints);                                                                     \
                                                                                                 \
		return PREFIX##_write_##LOWERTYPE(context, s, *(const UPPERTYPE*)v);                     \
	}                                                                                            \
                                                                                                 \
	const WinPrNdrMessageDescr PREFIX##_##LOWERTYPE##_descr_s = { WINPR_NDR_ARITY_SIMPLE,        \
		                                                          sizeof(UPPERTYPE),             \
		                                                          PREFIX##_read_##LOWERTYPE##_,  \
		                                                          PREFIX##_write_##LOWERTYPE##_, \
		                                                          NULL,                          \
		                                                          NULL };                        \
                                                                                                 \
...
#define ARRAY_OF_TYPE_IMPL(TYPE, UPPERTYPE)                                                        \
	BOOL winpr_ndr_read_##TYPE##Array(WinPrNdrContext* context, wStream* s, const void* hints,     \
	                                  void* v)                                                     \
	{                                                                                              \
		WINPR_ASSERT(context);                                                                     \
		WINPR_ASSERT(s);                                                                           \
		WINPR_ASSERT(hints);                                                                       \
		return winpr_ndr_read_uconformant_array(context, s, hints, winpr_ndr_##TYPE##_descr(), v); \
	}                                                                                              \
                                                                                                   \
	BOOL winpr_ndr_write_##TYPE##Array(WinPrNdrContext* context, wStream* s, const void* hints,    \
	                                   const void* v)                                              \
	{                                                                                              \
		WINPR_ASSERT(context);                                                                     \
		WINPR_ASSERT(s);                                                                           \
		...
		return winpr_ndr_write_uconformant_array(context, s, ahints->count,                        \
		                                         winpr_ndr_##TYPE##_descr(), v);                   \
	}                                                                                              \
	void winpr_ndr_destroy_##TYPE##Array(WinPrNdrContext* context, const void* hints, void* obj)   \
	{                                                                                              \
		WINPR_ASSERT(context);                                                                     \
		WINPR_ASSERT(obj);                                                                         \
		WINPR_ASSERT(hints);                                                                       \
	...
	const WinPrNdrMessageDescr winpr_ndr_##TYPE##Array_descr_s = {                                 \
		WINPR_NDR_ARITY_ARRAYOF,         sizeof(UPPERTYPE),                                        \
		winpr_ndr_read_##TYPE##Array,    winpr_ndr_write_##TYPE##Array,                            \
		winpr_ndr_destroy_##TYPE##Array, NULL                                                      \
	};                                                                                             \
                                                                                                   \
	...
	{                                                                                              \
		return &winpr_ndr_##TYPE##Array_descr_s;                                                   \
                                                                                                   ...
                                                                                                   \
	BOOL winpr_ndr_read_##TYPE##VaryingArray(WinPrNdrContext* context, wStream* s,                 \
	                                         const void* hints, void* v)                           \
	{                                                                                              \
		WINPR_ASSERT(context);                                                                     \
		WINPR_ASSERT(s);                                                                           \
		WINPR_ASSERT(hints);                                                                       \
		return winpr_ndr_read_uconformant_varying_array(                                           \
		    context, s, (const WinPrNdrVaryingArrayHints*)hints, winpr_ndr_##TYPE##_descr(), v);   \
	}                                                                                              \
	BOOL winpr_ndr_write_##TYPE##VaryingArray(WinPrNdrContext* context, wStream* s,                \
	                                          const void* hints, const void* v)                    \
	{                                                                                              \
		WINPR_ASSERT(context);                                                                     \
		WINPR_ASSERT(s);                                                                           \
		WINPR_ASSERT(hints);                                                                       \
		return winpr_ndr_write_uconformant_varying_array(                                          \
		    context, s, (const WinPrNdrVaryingArrayHints*)hints, winpr_ndr_##TYPE##_descr(), v);   \
	}                                                                                              \
                                                                                                   \
	const WinPrNdrMessageDescr winpr_ndr_##TYPE##VaryingArray_descr_s = {                          \
		...
		sizeof(UPPERTYPE),                                                                         \
		winpr_ndr_read_##TYPE##VaryingArray,                                                       \
		winpr_ndr_write_##TYPE##VaryingArray,                                                      \
		NULL,                                                                                      \
		NULL                                                                                       \
	};                                                                                             \
                                                                                                   \
...
BOOL winpr_ndr_read_wchar(WinPrNdrContext* context, wStream* s, WCHAR* ptr)
{
	return winpr_ndr_read_uint16(context, s, (UINT16*)ptr);
}

BOOL winpr_ndr_read_uconformant_varying_array(WinPrNdrContext* context, wStream* s,
                                              ...
                                              WinPrNdrMessageType itemType, void* ptarget)
{
	...
	if (!winpr_ndr_read_uint32(context, s, &maxCount) ||
	    !winpr_ndr_read_uint32(context, s, &offset) || !winpr_ndr_read_uint32(context, s, &length))
		return FALSE;
	...
	return winpr_ndr_read_align(context, s, 4);
}

BOOL winpr_ndr_write_uconformant_varying_array(WinPrNdrContext* context, wStream* s,
                                               const WinPrNdrVaryingArrayHints* hints,
                                               WinPrNdrMessageType itemType, const void* psrc)
{
	WINPR_ASSERT(context);
	WINPR_ASSERT(s);
	WINPR_ASSERT(hints);
	...
	if (!winpr_ndr_write_uint32(context, s, hints->maxLength) ||
	    !winpr_ndr_write_uint32(context, s, 0) ||
	    !winpr_ndr_write_uint32(context, s, hints->length))
		return FALSE;
...
BOOL winpr_ndr_read_uconformant_array(WinPrNdrContext* context, wStream* s,
                                      const WinPrNdrArrayHints* hints, WinPrNdrMessageType itemType,
                                      void* vtarget)
{
	WINPR_ASSERT(context);
	WINPR_ASSERT(s);
	...
	if (!winpr_ndr_read_uint32(context, s, &count))
		return FALSE;
	...
	return winpr_ndr_read_align(context, s, /*context->alignBytes*/ 4);
}

BOOL winpr_ndr_write_uconformant_array(WinPrNdrContext* context, wStream* s, UINT32 len,
                                       WinPrNdrMessageType itemType, const BYTE* ptr)
{
	...
	if (!winpr_ndr_write_uint32(context, s, len) ||
	...
	{
		Stream_Zero(s, padding);
		winpr_ndr_context_bytes_written(context, padding);
	}
	return TRUE;
}

BOOL winpr_ndr_struct_read_fromDescr(WinPrNdrContext* context, wStream* s,
...
#define NDR_MAX_STRUCT_DEFERRED 16
	...
	for (size_t i = 0; i < descr->nfields; i++)
	{
		...
		switch (field->pointerType)
		{
			...
			case WINPR_NDR_POINTER_NON_NULL:
			{
				...
				if (!winpr_ndr_read_refpointer(context, s, &deferred->ptrId))
				{
					WLog_ERR(TAG, "error when reading %s.%s", descr->name, field->name);
					return FALSE;
				}
	...
	return winpr_ndr_push_deferreds(context, deferreds, ndeferred);
}

BOOL winpr_ndr_struct_write_fromDescr(WinPrNdrContext* context, wStream* s,
	...
	for (size_t i = 0; i < descr->nfields; i++)
	{
		...
		switch (field->pointerType)
		{
			...
			case WINPR_NDR_POINTER_NON_NULL:
			{
				...
				if (!winpr_ndr_context_allocatePtr(context, ptr, &ptrId, &isNew))
					return FALSE;
				...
				if (!winpr_ndr_write_uint32(context, s, ptrId))
					return FALSE;
				break;
			}
			case WINPR_NDR_NOT_POINTER:
...
void winpr_ndr_struct_destroy(WinPrNdrContext* context, const WinPrNdrStructDescr* descr,
                              void* pptr)
{
	WINPR_ASSERT(context);
	WINPR_ASSERT(descr);
...
BOOL winpr_ndr_read_refpointer(WinPrNdrContext* context, wStream* s, winpr_ndr_refid* refId)
{
	return winpr_ndr_read_uint32(context, s, refId);
}
...
BOOL winpr_ndr_context_allocatePtr(WinPrNdrContext* context, const void* ptr,
...
BOOL winpr_ndr_read_pointedMessageEx(WinPrNdrContext* context, wStream* s, winpr_ndr_refid ptrId,
                                     WinPrNdrMessageType descr, void* hints, void** target)
{
	WINPR_ASSERT(context);
	WINPR_ASSERT(s);
	WINPR_ASSERT(descr);
...
BOOL winpr_ndr_push_deferreds(WinPrNdrContext* context, WinPrNdrDeferredEntry* deferreds,
                              size_t ndeferred)
{
	WINPR_ASSERT(context);
...
BOOL winpr_ndr_treat_deferred_read(WinPrNdrContext* context, wStream* s)
{
	WINPR_ASSERT(context);
	WINPR_ASSERT(s);
	...
	while (context->ndeferred)
	{
		...
		if (!winpr_ndr_read_pointedMessageEx(context, s, current.ptrId, current.msg, current.hints,
		                                     (void**)current.target))
		{
...
BOOL winpr_ndr_treat_deferred_write(WinPrNdrContext* context, wStream* s)
{
	WINPR_ASSERT(context);
	WINPR_ASSERT(s);
...
BOOL winpr_ndr_write_data(WinPrNdrContext* context, wStream* s, const void* data, size_t sz)
{
	if (!Stream_EnsureRemainingCapacity(s, sz))
		return FALSE;

	Stream_Write(s, data, sz);
	winpr_ndr_context_bytes_written(context, sz);
	return TRUE;
}
