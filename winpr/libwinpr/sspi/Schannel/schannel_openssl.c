...
int schannel_openssl_client_init(SCHANNEL_OPENSSL* context)
{
	...
	SSL_CTX_set_options(context->ctx, WINPR_SAFE_INT_CAST(uint64_t, options));
                                                       ...
                                                       PSecBufferDesc pOutput)
{
	...
	if (!context->connected)
	{
		...
		if (status > 0)
		{
			if (pBuffer->cbBuffer < WINPR_SAFE_INT_CAST(uint32_t, status))
				return SEC_E_INSUFFICIENT_MEMORY;

			CopyMemory(pBuffer->pvBuffer, context->ReadBuffer,
			           WINPR_SAFE_INT_CAST(uint32_t, status));
			pBuffer->cbBuffer = WINPR_SAFE_INT_CAST(uint32_t, status);
...
SECURITY_STATUS schannel_openssl_decrypt_message(SCHANNEL_OPENSSL* context, PSecBufferDesc pMessage)
{
	...
	pMessage->pBuffers[1].cbBuffer = WINPR_SAFE_INT_CAST(uint32_t, length);
