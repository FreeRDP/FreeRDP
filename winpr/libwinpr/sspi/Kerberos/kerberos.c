    ...
    PTimeStamp ptsExpiry)
{
	...
	krb5_cc_get_full_name(ctx, ccache, &ccache_setting.value);
...
cleanup:
	...
	if (ccache_setting.value)
		krb5_free_string(ctx, ccache_setting.value);
	if (principal)
    ...
    PCtxtHandle phNewContext, PSecBufferDesc pOutput, ULONG* pfContextAttr, PTimeStamp ptsExpiry)
{
	...
	UINT32 major, minor;
	UINT32 actual_services, actual_time;
	...
	major = sspi_gss_init_sec_context(
	    &minor, gss_creds, &context->gss_ctx, context->target_name, SSPI_GSS_C_SPNEGO_KRB5,
