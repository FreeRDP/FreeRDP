...
typedef enum e_SECURITY_IMPERSONATION_LEVEL
{
	SecurityAnonymous,
...
typedef struct s_LUID_AND_ATTRIBUTES
{
	LUID Luid;
	DWORD Attributes;
...
typedef struct s_SID_IDENTIFIER_AUTHORITY
{
	BYTE Value[6];
...
typedef struct s_SID_AND_ATTRIBUTES
{
	PSID Sid;
	DWORD Attributes;
...
typedef struct s_SID_AND_ATTRIBUTES_HASH
{
	DWORD SidCount;
...
typedef enum e_TOKEN_ELEVATION_TYPE
{
...
typedef enum e_TOKEN_INFORMATION_CLASS
{
	TokenUser = 1,
	TokenGroups,
	TokenPrivileges,
...
typedef struct s_TOKEN_PRIVILEGES
{
	DWORD PrivilegeCount;
	LUID_AND_ATTRIBUTES Privileges[ANYSIZE_ARRAY];
} TOKEN_PRIVILEGES, *PTOKEN_PRIVILEGES;

typedef struct s_TOKEN_OWNER
{
	PSID Owner;
} TOKEN_OWNER, *PTOKEN_OWNER;

typedef struct s_TOKEN_PRIMARY_GROUP
{
	PSID PrimaryGroup;
} TOKEN_PRIMARY_GROUP, *PTOKEN_PRIMARY_GROUP;

typedef struct s_TOKEN_DEFAULT_DACL
{
	PACL DefaultDacl;
} TOKEN_DEFAULT_DACL, *PTOKEN_DEFAULT_DACL;

typedef struct s_TOKEN_USER_CLAIMS
{
	PCLAIMS_BLOB UserClaims;
} TOKEN_USER_CLAIMS, *PTOKEN_USER_CLAIMS;

typedef struct s_TOKEN_DEVICE_CLAIMS
{
	PCLAIMS_BLOB DeviceClaims;
} TOKEN_DEVICE_CLAIMS, *PTOKEN_DEVICE_CLAIMS;

typedef struct s_TOKEN_GROUPS_AND_PRIVILEGES
{
	DWORD SidCount;
	DWORD SidLength;
...
typedef struct s_TOKEN_LINKED_TOKEN
{
	HANDLE LinkedToken;
} TOKEN_LINKED_TOKEN, *PTOKEN_LINKED_TOKEN;

typedef struct s_TOKEN_ELEVATION
{
	DWORD TokenIsElevated;
} TOKEN_ELEVATION, *PTOKEN_ELEVATION;

typedef struct s_TOKEN_MANDATORY_LABEL
{
	SID_AND_ATTRIBUTES Label;
} TOKEN_MANDATORY_LABEL, *PTOKEN_MANDATORY_LABEL;

typedef struct s_TOKEN_MANDATORY_POLICY
{
	DWORD Policy;
} TOKEN_MANDATORY_POLICY, *PTOKEN_MANDATORY_POLICY;

typedef struct s_TOKEN_ACCESS_INFORMATION
{
...
typedef struct s_TOKEN_AUDIT_POLICY
{
...
typedef struct s_TOKEN_STATISTICS
{
	LUID TokenId;
	LUID AuthenticationId;
...
typedef struct s_TOKEN_CONTROL
{
	LUID TokenId;
	LUID AuthenticationId;
	LUID ModifiedId;
...
typedef struct s_TOKEN_ORIGIN
{
	LUID OriginatingLogonSession;
} TOKEN_ORIGIN, *PTOKEN_ORIGIN;

typedef enum e_MANDATORY_LEVEL
{
...
typedef struct s_TOKEN_APPCONTAINER_INFORMATION
{
