...
#include <winpr/wlog.h>
	...
	WINPR_API WINPR_ATTR_NODISCARD void* winpr_backtrace(DWORD size);

	WINPR_ATTR_MALLOC(free, 1)
	WINPR_API WINPR_ATTR_NODISCARD char** winpr_backtrace_symbols(void* buffer, size_t* used);

	WINPR_API void winpr_backtrace_symbols_fd(void* buffer, int fd);
	WINPR_API WINPR_ATTR_NODISCARD char* winpr_strerror(INT32 dw, char* dmsg, size_t size);
