 ...
 */
#ifdef __cplusplus
#define WINPR_CXX_SAFE_CAST(t, val) static_cast<t>(val)
#else
#define WINPR_CXX_SAFE_CAST(t, val) (t)(val)
#endif
 ...
 * @since version 3.10.1
 */
#define WINPR_SAFE_INT_CAST(type, var)                                                      \
	__extension__({                                                                         \
		WINPR_ASSERT((var) ==                                                               \
		             WINPR_CXX_SAFE_CAST(__typeof(var), WINPR_CXX_SAFE_CAST(type, (var)))); \
		WINPR_ASSERT((((var) > 0) && (WINPR_CXX_SAFE_CAST(type, (var)) > 0)) ||             \
		             (((var) <= 0) && WINPR_CXX_SAFE_CAST(type, (var)) <= 0));              \
		WINPR_CXX_SAFE_CAST(type, (var));                                                   \
	})

#else
#define WINPR_SAFE_INT_CAST(type, var) WINPR_CXX_SAFE_CAST(type, var)
#endif
