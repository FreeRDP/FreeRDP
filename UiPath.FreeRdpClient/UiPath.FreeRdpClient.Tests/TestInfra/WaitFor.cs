...
internal class WaitFor
{
    ...
    public static async Task Predicate(Func<bool> predicate, TimeSpan timeout, bool throwOnTimeout = true, [CallerArgumentExpression(nameof(predicate))] string? expression = null)
    => await Predicate(predicate: () => Task.FromResult(predicate()), timeout: timeout, throwOnTimeout: throwOnTimeout, expression: expression);

    public static async Task Predicate(Func<Task<bool>> predicate, TimeSpan timeout, bool throwOnTimeout = true, [CallerArgumentExpression(nameof(predicate))] string? expression = null)
    ...
    public static async Task Predicate(Func<bool> predicate, CancellationToken cancelToken = default, bool throwOnTimeout = true, [CallerArgumentExpression(nameof(predicate))] string? expression = null)
    ...
    public static async Task Predicate(Func<Task<bool>> predicate, CancellationToken cancelToken = default, bool throwOnTimeout = true, [CallerArgumentExpression(nameof(predicate))] string? expression = null)
    {
