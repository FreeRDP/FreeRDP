...
extern "C"
{
	...
	FREERDP_API WINPR_ATTR_NODISCARD char* crypto_base64_encode(const BYTE* WINPR_RESTRICT data,
	                                                            size_t length);
	 ...
	 */
	FREERDP_API WINPR_ATTR_NODISCARD char* crypto_base64_encode_ex(const BYTE* WINPR_RESTRICT data,
	...
	FREERDP_API WINPR_ATTR_NODISCARD char* crypto_base64url_encode(const BYTE* WINPR_RESTRICT data,
	                                                               size_t length);
	FREERDP_API void crypto_base64url_decode(const char* WINPR_RESTRICT enc_data, size_t length,
	                                         BYTE** WINPR_RESTRICT dec_data,
	                                         size_t* WINPR_RESTRICT res_length);

	FREERDP_API WINPR_ATTR_NODISCARD char* crypto_read_pem(const char* WINPR_RESTRICT filename,
	                                                       size_t* WINPR_RESTRICT plength);
	FREERDP_API WINPR_ATTR_NODISCARD BOOL crypto_write_pem(const char* WINPR_RESTRICT filename,
