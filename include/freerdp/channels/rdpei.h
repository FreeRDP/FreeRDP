...
struct s_RDPINPUT_CONTACT_DATA
{
	UINT32 contactId;
...
typedef struct s_RDPINPUT_CONTACT_DATA RDPINPUT_CONTACT_DATA;

/** @brief a frame containing contact points */
struct s_RDPINPUT_TOUCH_FRAME
{
	UINT16 contactCount;
	UINT64 frameOffset;
	RDPINPUT_CONTACT_DATA* contacts;
};
typedef struct s_RDPINPUT_TOUCH_FRAME RDPINPUT_TOUCH_FRAME;

/** @brief a touch event with some frames*/
struct s_RDPINPUT_TOUCH_EVENT
{
	UINT32 encodeTime;
	UINT16 frameCount;
	RDPINPUT_TOUCH_FRAME* frames;
};
typedef struct s_RDPINPUT_TOUCH_EVENT RDPINPUT_TOUCH_EVENT;

struct s_RDPINPUT_PEN_CONTACT
{
	UINT8 deviceId;
...
typedef struct s_RDPINPUT_PEN_CONTACT RDPINPUT_PEN_CONTACT;

struct s_RDPINPUT_PEN_FRAME
{
	UINT16 contactCount;
	UINT64 frameOffset;
	RDPINPUT_PEN_CONTACT* contacts;
};
typedef struct s_RDPINPUT_PEN_FRAME RDPINPUT_PEN_FRAME;

/** @brief a touch event with some frames*/
struct s_RDPINPUT_PEN_EVENT
{
	UINT32 encodeTime;
	UINT16 frameCount;
	RDPINPUT_PEN_FRAME* frames;
};
typedef struct s_RDPINPUT_PEN_EVENT RDPINPUT_PEN_EVENT;
