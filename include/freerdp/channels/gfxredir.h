...
struct s_GFXREDIR_HEADER
{
	UINT32 cmdId;
	UINT32 length;
};

typedef struct s_GFXREDIR_HEADER GFXREDIR_HEADER;

struct s_GFXREDIR_LEGACY_CAPS_PDU
{
	UINT16 version; // GFXREDIR_CHANNEL_VERSION_LEGACY
};

typedef struct s_GFXREDIR_LEGACY_CAPS_PDU GFXREDIR_LEGACY_CAPS_PDU;

struct s_GFXREDIR_CAPS_HEADER
{
...
typedef struct s_GFXREDIR_CAPS_HEADER GFXREDIR_CAPS_HEADER;

struct s_GFXREDIR_CAPS_V2_0_PDU
{
	GFXREDIR_CAPS_HEADER header;
	UINT32 supportedFeatures; /* Reserved for future extensions */
};

typedef struct s_GFXREDIR_CAPS_V2_0_PDU GFXREDIR_CAPS_V2_0_PDU;

struct s_GFXREDIR_ERROR_PDU
{
	UINT32 errorCode;
};

typedef struct s_GFXREDIR_ERROR_PDU GFXREDIR_ERROR_PDU;

struct s_GFXREDIR_CAPS_ADVERTISE_PDU
{
	UINT32 length;    // length of caps;
	const BYTE* caps; // points variable length array of GFXREDIR_CAPS_HEADER.
};

typedef struct s_GFXREDIR_CAPS_ADVERTISE_PDU GFXREDIR_CAPS_ADVERTISE_PDU;

struct s_GFXREDIR_CAPS_CONFIRM_PDU
{
...
typedef struct s_GFXREDIR_CAPS_CONFIRM_PDU GFXREDIR_CAPS_CONFIRM_PDU;

struct s_GFXREDIR_OPEN_POOL_PDU
{
	UINT64 poolId;
	UINT64 poolSize;
...
typedef struct s_GFXREDIR_OPEN_POOL_PDU GFXREDIR_OPEN_POOL_PDU;

struct s_GFXREDIR_CLOSE_POOL_PDU
{
	UINT64 poolId;
};

typedef struct s_GFXREDIR_CLOSE_POOL_PDU GFXREDIR_CLOSE_POOL_PDU;

struct s_GFXREDIR_CREATE_BUFFER_PDU
{
	UINT64 poolId;
	UINT64 bufferId;
	UINT64 offset;
...
typedef struct s_GFXREDIR_CREATE_BUFFER_PDU GFXREDIR_CREATE_BUFFER_PDU;

struct s_GFXREDIR_DESTROY_BUFFER_PDU
{
	UINT64 bufferId;
};

typedef struct s_GFXREDIR_DESTROY_BUFFER_PDU GFXREDIR_DESTROY_BUFFER_PDU;

struct s_GFXREDIR_PRESENT_BUFFER_PDU
{
	UINT64 timestamp;
	UINT64 presentId;
	UINT64 windowId;
...
typedef struct s_GFXREDIR_PRESENT_BUFFER_PDU GFXREDIR_PRESENT_BUFFER_PDU;

struct s_GFXREDIR_PRESENT_BUFFER_ACK_PDU
{
	UINT64 windowId;
	UINT64 presentId;
};

typedef struct s_GFXREDIR_PRESENT_BUFFER_ACK_PDU GFXREDIR_PRESENT_BUFFER_ACK_PDU;
