...
struct s_RAIL_UNICODE_STRING
{
	UINT16 length;
	BYTE* string;
};
typedef struct s_RAIL_UNICODE_STRING RAIL_UNICODE_STRING;

struct s_RAIL_HIGH_CONTRAST
{
	UINT32 flags;
	UINT32 colorSchemeLength;
	RAIL_UNICODE_STRING colorScheme;
};
typedef struct s_RAIL_HIGH_CONTRAST RAIL_HIGH_CONTRAST;

/* RAIL Orders */

struct s_RAIL_HANDSHAKE_ORDER
{
	UINT32 buildNumber;
};
typedef struct s_RAIL_HANDSHAKE_ORDER RAIL_HANDSHAKE_ORDER;

struct s_RAIL_HANDSHAKE_EX_ORDER
{
	UINT32 buildNumber;
	UINT32 railHandshakeFlags;
};
typedef struct s_RAIL_HANDSHAKE_EX_ORDER RAIL_HANDSHAKE_EX_ORDER;

struct s_RAIL_CLIENT_STATUS_ORDER
{
	UINT32 flags;
};
typedef struct s_RAIL_CLIENT_STATUS_ORDER RAIL_CLIENT_STATUS_ORDER;

struct s_RAIL_EXEC_ORDER
{
	UINT16 flags;
...
typedef struct s_RAIL_EXEC_ORDER RAIL_EXEC_ORDER;

struct s_RAIL_EXEC_RESULT_ORDER
{
	UINT16 flags;
	UINT16 execResult;
	UINT32 rawResult;
...
typedef struct s_RAIL_EXEC_RESULT_ORDER RAIL_EXEC_RESULT_ORDER;

struct s_TS_FILTERKEYS
{
	UINT32 Flags;
	UINT32 WaitTime;
	UINT32 DelayTime;
...
typedef struct s_TS_FILTERKEYS TS_FILTERKEYS;

struct s_RAIL_SYSPARAM_ORDER
{
	UINT32 param;
	UINT32 params;
...
typedef struct s_RAIL_SYSPARAM_ORDER RAIL_SYSPARAM_ORDER;

struct s_RAIL_ACTIVATE_ORDER
{
	UINT32 windowId;
	BOOL enabled;
};
typedef struct s_RAIL_ACTIVATE_ORDER RAIL_ACTIVATE_ORDER;

struct s_RAIL_SYSMENU_ORDER
{
	UINT32 windowId;
	INT16 left;
	INT16 top;
};
typedef struct s_RAIL_SYSMENU_ORDER RAIL_SYSMENU_ORDER;

struct s_RAIL_SYSCOMMAND_ORDER
{
	UINT32 windowId;
	UINT16 command;
};
typedef struct s_RAIL_SYSCOMMAND_ORDER RAIL_SYSCOMMAND_ORDER;

struct s_RAIL_NOTIFY_EVENT_ORDER
{
	UINT32 windowId;
	UINT32 notifyIconId;
	UINT32 message;
};
typedef struct s_RAIL_NOTIFY_EVENT_ORDER RAIL_NOTIFY_EVENT_ORDER;

struct s_RAIL_MINMAXINFO_ORDER
{
	UINT32 windowId;
	INT16 maxWidth;
	INT16 maxHeight;
...
typedef struct s_RAIL_MINMAXINFO_ORDER RAIL_MINMAXINFO_ORDER;

struct s_RAIL_LOCALMOVESIZE_ORDER
{
	UINT32 windowId;
...
typedef struct s_RAIL_LOCALMOVESIZE_ORDER RAIL_LOCALMOVESIZE_ORDER;

struct s_RAIL_WINDOWMOVE_ORDER
{
	UINT32 windowId;
	INT16 left;
	INT16 top;
...
typedef struct s_RAIL_WINDOWMOVE_ORDER RAIL_WINDOW_MOVE_ORDER;

struct s_RAIL_GET_APPID_REQ_ORDER
{
	UINT32 windowId;
};
typedef struct s_RAIL_GET_APPID_REQ_ORDER RAIL_GET_APPID_REQ_ORDER;

struct s_RAIL_GET_APPID_RESP_ORDER
{
	UINT32 windowId;
	WCHAR applicationId[260];
};
typedef struct s_RAIL_GET_APPID_RESP_ORDER RAIL_GET_APPID_RESP_ORDER;

struct s_RAIL_LANGBAR_INFO_ORDER
{
	UINT32 languageBarStatus;
};
typedef struct s_RAIL_LANGBAR_INFO_ORDER RAIL_LANGBAR_INFO_ORDER;

struct s_RAIL_COMPARTMENT_INFO_ORDER
{
	UINT32 ImeState;
	UINT32 ImeConvMode;
...
typedef struct s_RAIL_COMPARTMENT_INFO_ORDER RAIL_COMPARTMENT_INFO_ORDER;

struct s_RAIL_ZORDER_SYNC
{
	UINT32 windowIdMarker;
};
typedef struct s_RAIL_ZORDER_SYNC RAIL_ZORDER_SYNC;
...
typedef struct s_RAIL_CLOAK RAIL_CLOAK;

struct s_RAIL_POWER_DISPLAY_REQUEST
{
	UINT32 active;
};
typedef struct s_RAIL_POWER_DISPLAY_REQUEST RAIL_POWER_DISPLAY_REQUEST;

struct s_RAIL_TASKBAR_INFO_ORDER
{
	UINT32 TaskbarMessage;
	UINT32 WindowIdTab;
	UINT32 Body;
};
typedef struct s_RAIL_TASKBAR_INFO_ORDER RAIL_TASKBAR_INFO_ORDER;

struct s_RAIL_LANGUAGEIME_INFO_ORDER
{
	UINT32 ProfileType;
	UINT32 LanguageID;
...
typedef struct s_RAIL_LANGUAGEIME_INFO_ORDER RAIL_LANGUAGEIME_INFO_ORDER;

struct s_RAIL_SNAP_ARRANGE
{
	UINT32 windowId;
	INT16 left;
	INT16 top;
...
typedef struct s_RAIL_SNAP_ARRANGE RAIL_SNAP_ARRANGE;

struct s_RAIL_GET_APPID_RESP_EX
{
	UINT32 windowID;
...
typedef struct s_RAIL_GET_APPID_RESP_EX RAIL_GET_APPID_RESP_EX;
