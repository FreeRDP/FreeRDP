...
struct s_POINTER_POSITION_UPDATE
{
	UINT32 xPos;
	UINT32 yPos;
};
typedef struct s_POINTER_POSITION_UPDATE POINTER_POSITION_UPDATE;

struct s_POINTER_SYSTEM_UPDATE
{
	UINT32 type;
};
typedef struct s_POINTER_SYSTEM_UPDATE POINTER_SYSTEM_UPDATE;

struct s_POINTER_COLOR_UPDATE
{
	UINT32 cacheIndex;
	UINT32 xPos;
	UINT32 yPos;
...
typedef struct s_POINTER_COLOR_UPDATE POINTER_COLOR_UPDATE;

struct s_POINTER_LARGE_UPDATE
{
	UINT16 xorBpp;
	UINT16 cacheIndex;
	UINT16 hotSpotX;
...
typedef struct s_POINTER_LARGE_UPDATE POINTER_LARGE_UPDATE;

struct s_POINTER_NEW_UPDATE
{
	UINT32 xorBpp;
	POINTER_COLOR_UPDATE colorPtrAttr;
};
typedef struct s_POINTER_NEW_UPDATE POINTER_NEW_UPDATE;

struct s_POINTER_CACHED_UPDATE
{
	UINT32 cacheIndex;
};
typedef struct s_POINTER_CACHED_UPDATE POINTER_CACHED_UPDATE;
