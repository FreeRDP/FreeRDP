...
extern "C"
{
	...
	FREERDP_API WINPR_ATTR_NODISCARD BOOL yuv420_context_decode(
	...
	FREERDP_API WINPR_ATTR_NODISCARD BOOL yuv420_context_encode(
	    YUV_CONTEXT* WINPR_RESTRICT context, const BYTE* WINPR_RESTRICT rgbData, UINT32 srcStep,
	    UINT32 srcFormat, const UINT32 iStride[3], BYTE* WINPR_RESTRICT yuvData[3],
	    const RECTANGLE_16* WINPR_RESTRICT regionRects, UINT32 numRegionRects);

	FREERDP_API WINPR_ATTR_NODISCARD BOOL yuv444_context_decode(
	    ...
	    const RECTANGLE_16* WINPR_RESTRICT regionRects, UINT32 numRegionRects);
	FREERDP_API WINPR_ATTR_NODISCARD BOOL yuv444_context_encode(
	    YUV_CONTEXT* WINPR_RESTRICT context, BYTE version, const BYTE* WINPR_RESTRICT pSrcData,
	    UINT32 nSrcStep, UINT32 SrcFormat, const UINT32 iStride[3],
	    BYTE* WINPR_RESTRICT pYUVLumaData[3], BYTE* WINPR_RESTRICT pYUVChromaData[3],
	    const RECTANGLE_16* WINPR_RESTRICT regionRects, UINT32 numRegionRects);

	FREERDP_API WINPR_ATTR_NODISCARD BOOL yuv_context_reset(YUV_CONTEXT* WINPR_RESTRICT context,
	                                                        UINT32 width, UINT32 height);

	FREERDP_API void yuv_context_free(YUV_CONTEXT* context);

	WINPR_ATTR_MALLOC(yuv_context_free, 1)
	FREERDP_API WINPR_ATTR_NODISCARD YUV_CONTEXT* yuv_context_new(BOOL encoder,
