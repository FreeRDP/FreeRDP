...
set(OSSL_CFLAGS
    "-target ${OSSL_TARGET} --sysroot=${OHOS_SYSROOT} -D__MUSL__ ${OSSL_CFLAGS_EXTRA}"
)

set(HARMONY_OPENSSL_PATCH
    "${CMAKE_CURRENT_LIST_DIR}/../../../scripts/harmony-openssl.patch")
...
ExternalProject_Add(
  ...
  CONFIGURE_COMMAND
    ${CMAKE_COMMAND} -E env ${OSSL_ENV} perl <SOURCE_DIR>/Configure ${OSSL_ARCH}
    no-shared no-module no-tests no-apps no-docs no-legacy no-dso no-ssl-trace
    --prefix=${OPENSSL_INSTALL_DIR} --libdir=${OPENSSL_INSTALL_DIR}
  ...
  COMMAND ${CMAKE_COMMAND} -E copy_directory <SOURCE_DIR>/include/openssl
          ${OPENSSL_INSTALL_DIR}/include/openssl
  COMMAND ${CMAKE_COMMAND} -E copy_directory <BINARY_DIR>/include/openssl
          ${OPENSSL_INSTALL_DIR}/include/openssl
  COMMAND ${CMAKE_COMMAND} -E copy <BINARY_DIR>/libssl.a
          ${OPENSSL_INSTALL_DIR}/libssl.a
  COMMAND ${CMAKE_COMMAND} -E copy <BINARY_DIR>/libcrypto.a
          ${OPENSSL_INSTALL_DIR}/libcrypto.a
  BUILD_BYPRODUCTS ${OPENSSL_INSTALL_DIR}/libssl.a
                   ${OPENSSL_INSTALL_DIR}/libcrypto.a)
...
set_target_properties(
  ...
  PROPERTIES
    ...
    "${OPENSSL_INSTALL_DIR}/libssl.a;${OPENSSL_INSTALL_DIR}/libcrypto.a")
