...
{
	/* Destroy the Wayland resize band before the SDL window (and its parent surface) goes away. */
	sdl_wayland_band_remove(window());
}
...
bool SdlRailWindow::reconcile(SDL_Window* parent, const SDL_Rect& parentRect)
{
	...
	lock.unlock();
	/* Track the Wayland outside resize band to the current insets (self-locks; no-op on X11). */
	syncWaylandBand();
	return true;
}

void SdlRailWindow::syncWaylandBand()
{
	/* Wayland only (positions unreadable there); zero per-frame cost on X11. */
	if (railPlatformCaps().positionsReadable)
		return;
	bool changed = false;
	{
		...
		if (!_win || !_win->window())
			return;
		/* bandMargins() already floors each edge to a min grip and is zero for
		 * non-resizable/popup/maximized windows. insets()/reported margins are untouched. */
		const SDL_Rect m = bandMargins();
		changed = sdl_wayland_band_sync(_win->window(), m.x, m.y, m.w, m.h);
	}
	/* A changed band repositions on the next parent commit: force a present. */
	if (changed)
		invalidateAll();
}
