...
static BOOL sdl_apply_display_properties(SdlContext* sdl)
{
	...
	for (UINT32 x = 0; x < numIds; x++)
	{
		...
		float dpi = SDL_GetDisplayContentScale(WINPR_SAFE_INT_CAST(uint32_t, *id));
		float hdpi = dpi;
		float vdpi = dpi;
		SDL_Rect rect = {};

		if (!SDL_GetDisplayBounds(WINPR_SAFE_INT_CAST(uint32_t, *id), &rect))
			return FALSE;
		...
		const SDL_DisplayOrientation orientation =
		    SDL_GetCurrentDisplayOrientation(WINPR_SAFE_INT_CAST(uint32_t, *id));
...
static BOOL sdl_detect_single_window(SdlContext* sdl, UINT32* pMaxWidth, UINT32* pMaxHeight)
{
	     ...
	     !freerdp_settings_get_bool(settings, FreeRDP_RemoteApplicationMode)))
	{
		...
		if (freerdp_settings_get_uint32(settings, FreeRDP_NumMonitorIds) == 0)
		{
			const size_t id =
			    (!sdl->windows.empty())
			        ? WINPR_SAFE_INT_CAST(size_t, sdl->windows.begin()->second.displayIndex())
			        : 0;
