...
class SdlRail
{
  public:
	...
	/* Wayland (main thread): a press on the outside resize band (sdl_wayland_band) starts the
	 * compositor resize for its `xdgEdge` (XDG_TOPLEVEL_RESIZE_EDGE_*) and sets up completion. */
	[[nodiscard]] bool beginBandResize(SDL_WindowID id, uint32_t xdgEdge);
  ...
  private:
	...
	/* Set up the client-driven resize completion state for a band-started compositor resize;
	 * caller holds _windowsLock. */
	bool beginClientEdgeResize(SdlRailWindow* appWindow, uint16_t edge);
