...
bool SdlWindow::paintResizeFrame(SDL_Surface* surface, SDL_Point off, bool contentChanged)
{
	...
	/* Skip the full-surface upload unless the texture was recreated or content changed. */
	const bool cached = _gdiTexture && (_gdiTextureW == surface->w) && (_gdiTextureH == surface->h);
	if (!ensureGdiTexture(surface))
		return false;
