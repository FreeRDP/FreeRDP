...
BOOL sdl_auth_dialog_show(const SDL_UserAuthArg* args)
{
	...
	const std::vector<std::string> fidoTouch = { "Touch the security key" };
	...
	if (!prompt.empty())
	{
		...
		else if (args->result == AUTH_FIDO_TOUCH)
		{
			initial = { "" };
			flags = { SdlInputWidgetPair::SDL_INPUT_READONLY };
		}
	...
	if (rc > 0)
	{
		...
		{
			/* No credentials needed, just acknowledgement */
		}
		else
		{
