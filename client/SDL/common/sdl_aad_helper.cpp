...
SdlAadAuthHelper::SdlAadAuthHelper(AadAuthHelper* helper) : _helper(helper)
{
}
...
#if defined(WITH_XDG_AAD_AUTH_HELPER) || defined(WITH_WEBVIEW_AAD_AUTH_HELPER) || \
    defined(WITH_QT_AAD_AUTH_HELPER)
#define SDL_AAD_HELPER_HAVE_CANDIDATES 1
...
#if defined(WITH_XDG_AAD_AUTH_HELPER)
		"freerdp-xdg-aad-helper",
#endif
#if defined(WITH_WEBVIEW_AAD_AUTH_HELPER)
		"freerdp-webview-aad-helper",
#endif
#if defined(WITH_QT_AAD_AUTH_HELPER)
...
#if defined(SDL_AAD_HELPER_HAVE_CANDIDATES)
...
static AadAuthHelper* sdl_aad_helper_get(rdpContext* context, SdlAadAuthHelperPtr& helper)
{
	if (helper)
		return helper->get();
	...
	if (!PathFileExistsA(path.c_str()))
	{
		WLog_ERR(
		    ...
		    "installed with -DWITH_XDG_AAD_AUTH_HELPER=ON, -DWITH_WEBVIEW_AAD_AUTH_HELPER=ON or "
		    "-DWITH_QT_AAD_AUTH_HELPER=ON? Falling back to manual copy/paste login",
		    path.c_str(), source);
		return nullptr;
	}

	WLog_DBG(TAG, "[aad-auth] using helper path from %s: %s", source, path.c_str());
	auto* raw = aad_auth_helper_start(path.c_str());
	if (!raw)
	{
	...
	helper = std::make_shared<SdlAadAuthHelper>(raw);
	return raw;
}
...
static AadAuthHelperNavigateStatus
sdl_aad_helper_navigate(rdpContext* context, SdlAadAuthHelperPtr& helper, const std::string& title,
                        const std::string& url, std::string& redirectUrl)
{
	...
	auto* rawHelper = sdl_aad_helper_get(context, helper);
	if (!rawHelper)
		return AAD_AUTH_HELPER_NAVIGATE_ERROR;

	char* out = nullptr;
	const AadAuthHelperNavigateStatus status = aad_auth_helper_navigate(
	    rawHelper, title.c_str(), url.c_str(), redirectUri.c_str(), 180000, &out);
...
static BOOL sdl_aad_helper_get_rdsaad_access_token(freerdp* instance, SdlAadAuthHelperPtr& helper,
                                                   ...
                                                   char** token)
{
	...
	const AadAuthHelperNavigateStatus status = sdl_aad_helper_navigate(
	    context, helper, "FreeRDP WebView - AAD access token", request.get(), redirectUrl);
...
static BOOL sdl_aad_helper_get_avd_access_token(freerdp* instance, SdlAadAuthHelperPtr& helper,
                                                char** token)
{
	WINPR_ASSERT(token);
	WINPR_ASSERT(instance);
	...
	const AadAuthHelperNavigateStatus status =
	    sdl_aad_helper_navigate(instance->context, helper, "FreeRDP WebView - AVD access token",
...
BOOL sdl_aad_helper_get_access_token_v(freerdp* instance, SdlAadAuthHelperPtr& helper,
                                       ...
                                       va_list args)
{
	...
	switch (tokenType)
	{
		case ACCESS_TOKEN_TYPE_AAD:
		{
			...
			return sdl_aad_helper_get_rdsaad_access_token(instance, helper, scope, req_cnf, token);
		}
		case ACCESS_TOKEN_TYPE_AVD:
			if (count != 0)
				WLog_WARN(TAG,
				          ...
				          ", ignoring",
				          count);
			return sdl_aad_helper_get_avd_access_token(instance, helper, token);
		default:
...
BOOL sdl_aad_helper_get_access_token(freerdp* instance, SdlAadAuthHelperPtr& helper,
                                     AccessTokenType tokenType, char** token, size_t count, ...)
{
	va_list ap = {};
	va_start(ap, count);
	const BOOL rc =
	    sdl_aad_helper_get_access_token_v(instance, helper, tokenType, token, count, ap);
	va_end(ap);
	return rc;
}
