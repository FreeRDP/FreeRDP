                             ...
                             Sint32 width, Sint32 height)
{
	...
	_list.clear();
	size_t button_width = ids.size() * (WINPR_SAFE_INT_CAST(uint32_t, width) + hpadding) + hpadding;
	size_t offsetX = WINPR_SAFE_INT_CAST(uint32_t, total_width) -
	                 std::min<size_t>(WINPR_SAFE_INT_CAST(uint32_t, total_width), button_width);
