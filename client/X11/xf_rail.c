...
UINT X11_rail_server_start_cmd(RailClientContext* context)
{
	UINT status;
	...
	/**
	 * TS_RAIL_CLIENTSTATUS_ALLOWLOCALMOVESIZE This attribute is not well supported by the
	 * operating system above win7, which will lead to the connection of wiin8 /win10 system in
	 *  RAIL mode, and the application window will have picture defects and out of control drift
	 **/
...
{
	return X11_rail_server_start_cmd(context);
}
