                   ...
                   int evtype)
{
	...
	switch (evtype)
	{
		...
		case XI_ButtonRelease:
			...
			if (xfc->xi_event)
			{
				const XButtonEvent* bev = &xevent->xbutton;
				if (!xfc_is_floatbar_window(xfc, bev->window))
				{
		...
		case XI_Motion:
			...
			if (xfc->xi_event)
			{
				const XMotionEvent* mev = &xevent->xmotion;
				if (!xfc_is_floatbar_window(xfc, mev->window))
				{
