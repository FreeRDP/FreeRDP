...
static UINT xf_OutputUpdate(xfContext* xfc, xfGfxSurface* surface)
{
	...
	for (UINT32 x = 0; x < nbRects; x++)
	{
		...
		if (xfc->remote_app)
		{
			...
			(void)xf_rail_paint_surface(xfc, surface->gdi.windowId, rect);
		}
		else
#ifdef WITH_XRENDER
		    if (freerdp_settings_get_bool(settings, FreeRDP_SmartSizing) ||
...
static void DestroySurface(WINPR_ATTR_UNUSED xfContext* xfc, xfGfxSurface* surface)
{
	if (!surface)
		return;
...
#endif
#ifdef WITH_XSHM
	/* SHM data must be detached, not passed to the aligned heap allocator. */
	(void)xf_shm_destroy_surface_image(xfc, surface);
#endif
	if (surface->image)
	{
