...
static void xf_reconnect_draw(xfContext* xfc)
{
	...
	GC gc = LogDynAndXCreateGC(xfc->log, xfc->display, xfc->reconnectWindow, 0, nullptr);
	if (!gc)
		return;
	LogDynAndXSetForeground(xfc->log, xfc->display, gc, BlackPixel(xfc->display, screen));
	XFontStruct* font = XLoadQueryFont(xfc->display, "10x20");
	if (font)
		XSetFont(xfc->display, gc, font->fid);
	LogDynAndXClearWindow(xfc->log, xfc->display, xfc->reconnectWindow);
...
SSIZE_T xf_retry_dialog(freerdp* instance, const char* what, size_t current, void* userarg)
{
	...
	if (!xfc->reconnectWindow)
	{
		...
		LogDynAndXSelectInput(xfc->log, xfc->display, xfc->reconnectWindow, ExposureMask);
...
void xf_reconnect_close(xfContext* xfc)
{
	...
	{
		xf_lock_x11(xfc);
		LogDynAndXDestroyWindow(xfc->log, xfc->display, xfc->reconnectWindow);
