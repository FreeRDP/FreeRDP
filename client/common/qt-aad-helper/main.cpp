...
AuthWindow::AuthWindow(Session& owner) : session(owner)
{
	...
	connect(this, &QWebEngineView::loadFinished, this, &AuthWindow::onLoadFinished);
}
...
void AuthWindow::onLoadFinished(bool ok)
{
	std::cout << "[qt-aad-helper] loaded " << url().toString().toStdString()
	          << (ok ? "" : " (failed)") << std::endl;
}
...
namespace
{
	...
	void sendJson(const QJsonObject& obj)
	{
		...
		sendLine(doc.toJson(QJsonDocument::Compact).toStdString());
	}
	...
	void sendHelloResult(int64_t id)
	{
		...
		result["helper"] = QStringLiteral("freerdp-qt-aad-helper/1.0");
	...
	void sendNavigateResult(int64_t id, const std::string& redirectUrl)
	{
		...
		result["redirect_url"] = QString::fromStdString(redirectUrl);
	...
	void sendError(int64_t id, int code, const std::string& message)
	{
		QJsonObject error;
		error["code"] = code;
		error["message"] = QString::fromStdString(message);
	...
	std::string getStringField(const QJsonObject& obj, const char* name)
	{
		const QJsonValue v = obj.value(QLatin1String(name));
		return v.isString() ? v.toString().toStdString() : std::string();
	}

	uint32_t getUintField(const QJsonObject& obj, const char* name, uint32_t def)
	{
		const QJsonValue v = obj.value(QLatin1String(name));
		return v.isDouble() ? static_cast<uint32_t>(v.toDouble()) : def;
	}
	...
	void readerLoop(Session& session)
	{
		...
		while (readLine(buf, line))
		{
			...
			const QJsonValue idValue = msg.value(QLatin1String("id"));
			const bool hasId = idValue.isDouble();
			const int64_t id = hasId ? static_cast<int64_t>(idValue.toDouble()) : 0;
			const std::string method = getStringField(msg, "method");
			const QJsonObject params = msg.value(QLatin1String("params")).toObject();
