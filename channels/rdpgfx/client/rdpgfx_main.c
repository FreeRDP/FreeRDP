                                           ...
                                           const RDPGFX_CAPS_ADVERTISE_PDU* pdu)
{
	...
	for (UINT16 index = 0; index < pdu->capsSetCount; index++)
	{
		...
		WLog_Print(gfx->log, WLOG_DEBUG, "Sending %s [0x%08" PRIx32 "] flags=0x%08" PRIx32,
                                              ...
                                              const RDPGFX_FRAME_ACKNOWLEDGE_PDU* pdu)
{
	...
	WLog_Print(gfx->log, WLOG_TRACE, "SendFrameAcknowledgePdu: %" PRIu32 "", pdu->frameId);
                                                  ...
                                                  const RDPGFX_QOE_FRAME_ACKNOWLEDGE_PDU* pdu)
{
	...
	WLog_Print(gfx->log, WLOG_TRACE, "SendQoeFrameAcknowledgePdu: %" PRIu32 "", pdu->frameId);
	s = Stream_New(nullptr, header.pduLength);
...
static UINT rdpgfx_recv_cache_import_reply_pdu(GENERIC_CHANNEL_CALLBACK* callback, wStream* s)
{
	...
	WLog_Print(gfx->log, WLOG_TRACE, "RecvCacheImportReplyPdu: importedEntriesCount: %" PRIu16 "",
...
static UINT rdpgfx_recv_end_frame_pdu(GENERIC_CHANNEL_CALLBACK* callback, wStream* s)
{
	...
	Stream_Read_UINT32(s, pdu.frameId); /* frameId (4 bytes) */
	WLog_Print(gfx->log, WLOG_TRACE, "RecvEndFramePdu: frameId: %" PRIu32 "", pdu.frameId);
