...
static void fill_gfx_10_caps(RDPGFX_PLUGIN* gfx, RDPGFX_CAPSET* capsSet, UINT32 version)
{
...
static UINT rdpgfx_send_caps_advertise_pdu(RDPGFX_CHANNEL_CALLBACK* callback)
{
	...
	if (gfx->CapsFlags & GFX_CAPS_10)
		fill_gfx_10_caps(gfx, &capsSets[pdu.capsSetCount++], RDPGFX_CAPVERSION_10);

	if (gfx->CapsFlags & GFX_CAPS_102)
		fill_gfx_10_caps(gfx, &capsSets[pdu.capsSetCount++], RDPGFX_CAPVERSION_102);

	if (gfx->CapsFlags & GFX_CAPS_103)
		fill_gfx_10_caps(gfx, &capsSets[pdu.capsSetCount++], RDPGFX_CAPVERSION_103);
...
UINT DVCPluginEntry(IDRDYNVC_ENTRY_POINTS* pEntryPoints)
{
	...
	if (!gfx)
	{
		...
		gfx->CapsFlags = gfx->settings->GfxCapsFlags;
