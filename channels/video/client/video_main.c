...
static UINT video_read_tsmm_presentation_req(VideoClientContext* context, wStream* s)
{
	...
	WLog_DBG(TAG,
	         "presentationReq: id:%"PRIu8" version:%"PRIu8" command:%s srcWidth/srcHeight=%"PRIu32"x%"PRIu32
...
static UINT video_plugin_initialize(IWTSPlugin* plugin, IWTSVirtualChannelManager* channelMgr)
{
	...
	video->control_callback = callback = (VIDEO_LISTENER_CALLBACK*) calloc(1,
	                                     sizeof(VIDEO_LISTENER_CALLBACK));
	...
	video->data_callback = callback = (VIDEO_LISTENER_CALLBACK*) calloc(1,
	                                  sizeof(VIDEO_LISTENER_CALLBACK));
