...
static int urbdrc_process_register_request_callback(URBDRC_CHANNEL_CALLBACK*
        callback,
...
static int urbdrc_process_cancel_request(BYTE* data, UINT32 data_sizem,
...
static int urbdrc_process_retract_device_request(BYTE* data, UINT32 data_sizem,
        IUDEVMAN* udevman, UINT32 UsbDevice)
{
	UINT32 Reason;
	...
	switch (Reason)
	{
		...
		default:
			WLog_DBG(TAG, "urbdrc_process_retract_device_request: Unknown Reason %d",
			         Reason);
			return -1;
			break;
	}
                                     ...
                                     UINT32 data_sizem, UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice)
{
	...
	switch (IoControlCode)
	{
		...
		default:
			WLog_DBG(TAG, "urbdrc_process_io_control: unknown IoControlCode 0x%X",
			         IoControlCode);
			free(OutputBuffer);
        ...
        UINT32 data_sizem, UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice)
{
	...
	pdev->control_query_device_text(pdev, TextType, LocaleId, &bufferSize,
	                                DeviceDescription);
	...
	if (bufferSize != 0)
	{
		data_write_UINT32(out_data + 8,
		                  (bufferSize / 2) + 1); /** cchDeviceDescription */
		out_offset = 12;
                                    ...
                                    int transferDir)
{
	...
	if (ConfigurationDescriptorIsValid)
	{
		...
		MsConfig = msusb_msconfig_read(data + offset, data_sizem - offset,
		                               NumInterfaces);
		/* select config */
...
static int urb_select_interface(URBDRC_CHANNEL_CALLBACK* callback, BYTE* data,
                                UINT32 data_sizem,
                                UINT32 MessageId, IUDEVMAN* udevman, UINT32 UsbDevice, int transferDir)
{
	...
	out_offset = 8;
	MsInterface = msusb_msinterface_read(data + out_offset, data_sizem - out_offset,
	                                     &out_offset);
	data_read_UINT32(data + out_offset, OutputBufferSize);
	pdev->select_interface(pdev, MsInterface->InterfaceNumber,
                              ...
                              int transferDir)
{
	...
	data_write_UINT32(out_data + 12, RequestId);	/** RequestId */
	data_write_UINT32(out_data + 16,
	                  20 + (NumberOfPackets * 12));	/** CbTsUrbResult */
	/** TsUrbResult TS_URB_RESULT_HEADER */
	...
	data_write_UINT32(out_data + offset, 0);	/** HResult */
	data_write_UINT32(out_data + offset + 4,
	                  OutputBufferSize);	/** OutputBufferSize */
#if ISOCH_FIFO
...
static int urb_control_vendor_or_class_request(URBDRC_CHANNEL_CALLBACK*
        callback,
        BYTE* data,
        UINT32 data_sizem,
        UINT32 MessageId,
...
static int urb_control_get_configuration_request(URBDRC_CHANNEL_CALLBACK*
        callback,
        BYTE* data,
        UINT32 data_sizem,
        UINT32 MessageId,
