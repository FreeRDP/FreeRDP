...
static void udevman_parse_device_pid_vid(char* str, int* id1, int* id2,
        char sign)
{
	char s1[8];
	char* s2;
...
static int udevman_check_device_exist_by_id(IUDEVMAN* idevman, UINT16 idVendor,
        UINT16 idProduct)
{
        ...
        UINT32 UsbDevice)
{
	...
	idevman->loading_unlock(idevman);
	/* try again */
	pdev = (UDEVICE*) idevman->get_udevice_by_UsbDevice_try_again(idevman,
	        UsbDevice);
	...
	while (token)
	{
		...
		if (udevman->flags & UDEVMAN_FLAG_ADD_BY_VID_PID)
		{
			...
			success = udevman->iface.register_udevice((IUDEVMAN*) udevman,
			          0, 0, UsbDevice, (UINT16) idVendor, (UINT16) idProduct,
...
int freerdp_urbdrc_client_subsystem_entry(PFREERDP_URBDRC_SERVICE_ENTRY_POINTS
        pEntryPoints)
{
	UDEVMAN* udevman;
