...
static void* id_copy(const void* pv)
{
	const VID_PID_PAIR* p = pv;
	if (!p)
	{
		return NULL;
	}

	VID_PID_PAIR* copy = calloc(1, sizeof(VID_PID_PAIR));
	if (!copy)
	{
		return NULL;
	}
