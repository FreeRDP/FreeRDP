...
static const LIBUSB_ENDPOINT_DESCEIPTOR* func_get_ep_desc(
...
static void func_iso_data_init(ISO_USER_DATA* iso_user_data, UINT32 numPacket,
                               UINT32 buffsize,
...
static int func_config_release_all_interface(LIBUSB_DEVICE_HANDLE*
        libusb_handle, UINT32 NumInterfaces)
{
	int i, ret;
...
static LIBUSB_DEVICE_DESCRIPTOR* udev_new_descript(LIBUSB_DEVICE* libusb_dev)
{
	...
	descriptor = (LIBUSB_DEVICE_DESCRIPTOR*) malloc(sizeof(
...
static int udev_get_hub_handle(UDEVICE* pdev, UINT16 bus_number,
                               UINT16 dev_number)
{
	int error;
	...
	for (i = 0; i < total_device; i ++)
	{
		...
		/* get port number */
		error = libusb_get_port_numbers(libusb_list[i], port_numbers,
		                                sizeof(port_numbers));
		...
		if (error < 1)
		{
			...
			WLog_ERR(TAG, "libusb_get_port_numbers error: %i - %s", error,
	...
	if (error == 0)
	{
		...
		{
			if ((bus_number != libusb_get_bus_number(libusb_list[i])) ||
			    (1 != libusb_get_device_address(
			         libusb_list[i]))) /* Root hub allways first on bus. */
				continue;
...
static int libusb_udev_select_interface(IUDEVICE* idev, BYTE InterfaceNumber,
...
static MSUSB_CONFIG_DESCRIPTOR* libusb_udev_complete_msconfig_setup(
    IUDEVICE* idev, MSUSB_CONFIG_DESCRIPTOR* MsConfig)
{
	...
	for (inum = 0; inum < MsConfig->NumInterfaces; inum++)
	{
		...
		t_MsPipes = (MSUSB_PIPE_DESCRIPTOR**) malloc(LibusbNumEndpoint * sizeof(
...
static void libusb_udev_attach_kernel_driver(IUDEVICE* idev)
{
	...
	for (i = 0; i < pdev->LibusbConfig->bNumInterfaces
	     && err != LIBUSB_ERROR_NO_DEVICE; i++)
	{
...
static int libusb_udev_query_device_port_status(IUDEVICE* idev,
        UINT32* UsbdStatus, UINT32* BufferSize, BYTE* Buffer)
{
	UDEVICE* pdev = (UDEVICE*) idev;
	int success = 0, ret;
	WLog_DBG(TAG, "...");
...
static int libusb_udev_isoch_transfer(IUDEVICE* idev, UINT32 RequestId,
                                      ...
                                      BYTE* IsoPacket, UINT32* BufferSize, BYTE* Buffer, int Timeout)
{
	...
	func_iso_data_init(&iso_user_data, NumberOfPackets, *BufferSize, NoAck,
	                   IsoPacket, Buffer);
	/** fill setting */
...
static int libusb_udev_bulk_or_interrupt_transfer(IUDEVICE* idev,
        UINT32 RequestId,
        UINT32 EndpointAddress, UINT32 TransferFlags, UINT32* UsbdStatus,
        UINT32* BufferSize, BYTE* Buffer, UINT32 Timeout)
{
	...
	{
		if (pdev->request_queue->unregister_request(pdev->request_queue, RequestId))
			WLog_ERR(TAG,  "request_queue_unregister_request: not fount request 0x%x",
			         RequestId);
	}
...
static void udev_load_interface(UDEVICE* pdev)
{
	...
	pdev->iface.os_feature_descriptor_request =
	    libusb_udev_os_feature_descriptor_request;
	pdev->iface.cancel_all_transfer_request =
...
static IUDEVICE* udev_init(UDEVICE* pdev, UINT16 bus_number, UINT16 dev_number)
{
	...
	status = libusb_get_active_config_descriptor(pdev->libusb_dev,
	         &pdev->LibusbConfig);
