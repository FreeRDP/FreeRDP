	...
	for (i = 0; i < ARRAYSIZE(mszstring_enumerator_tests); i ++)
	{
		...
		while (mszStrings_Enumerator_MoveNext(& enumerator))
		{
			if (cmp(widechar, list[k], mszStrings_Enumerator_Current(& enumerator)) != 0)
			{
				...
				printf(" mszString[ %d ] =  ", k);
				memdump(mszStrings_Enumerator_Current(& enumerator), len(widechar,
				        mszStrings_Enumerator_Current(& enumerator)));
				printf("\n");
				printf("      list[ %d ] =  ", k);
				memdump(list[k], len(widechar, list[k]));
				printf("\n");
			}
...
BOOL test_ConvertFromUnicode_on_msz()
{
	...
	else
	{
		if (0 != memcmp(convert_from_unicode_on_msz_test.expected, result, actual_result_size))
		{
			...
			printf("\nmsz:      ");
			memdump(convert_from_unicode_on_msz_test.msz,
			        convert_from_unicode_on_msz_test.msz_size * sizeof(WCHAR));
			printf("\nresult:   ");
			memdump(result, result_size);
			printf("\nresult:   ");
			memdump(result, actual_result_size);
			printf("\nexpected: ");
			memdump(convert_from_unicode_on_msz_test.expected, convert_from_unicode_on_msz_test.expected_size);
			printf("\n");
		}
	}
