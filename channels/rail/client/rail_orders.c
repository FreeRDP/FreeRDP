...
UINT rail_send_client_window_move_order(railPlugin* rail, const RAIL_WINDOW_MOVE_ORDER* windowMove)
{
	...
	if (error == ERROR_SUCCESS)
	{
		WLog_Print(rail->log, WLOG_TRACE, "sending new RAIL window corners: 0x%x (%d, %d), (%d, %d)",
		           windowMove->windowId, windowMove->left, windowMove->top, windowMove->right, windowMove->bottom);
