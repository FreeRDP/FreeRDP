 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT remdesk_recv_ctl_version_info_pdu(wStream* s)
{
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT remdesk_recv_ctl_authenticate_pdu(wStream* s, REMDESK_CHANNEL_HEADER* header)
{
	int cchStringW = 0;
	UINT32 msgLength = 0;
	int cbExpertBlobW = 0;
                                 ...
                                 REMDESK_CHANNEL_HEADER* header)
{
	...
	switch (msgType)
	{
		...
		case REMDESK_CTL_AUTHENTICATE:
			if ((error = remdesk_recv_ctl_authenticate_pdu(s, header)))
			{
		...
		case REMDESK_CTL_VERSIONINFO:
			if ((error = remdesk_recv_ctl_version_info_pdu(s)))
			{
