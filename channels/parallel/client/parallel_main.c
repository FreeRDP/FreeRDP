...
} DECLSPEC_ALIGN(128) PARALLEL_DEVICE;
...
static UINT parallel_process_irp_write(PARALLEL_DEVICE* parallel, IRP* irp)
{
	...
	Stream_Read_UINT32(irp->input, Length);
	Stream_Read_UINT64(irp->input, Offset);
	if (!Stream_SafeSeek(irp->input, 20))
	{ /* Padding */
		return ERROR_INVALID_DATA;
	}
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT parallel_process_irp_device_control(IRP* irp)
{
...
static UINT parallel_process_irp(PARALLEL_DEVICE* parallel, IRP* irp)
{
	...
	switch (irp->MajorFunction)
	{
		...
		case IRP_MJ_DEVICE_CONTROL:
			if ((error = parallel_process_irp_device_control(irp)))
			{
