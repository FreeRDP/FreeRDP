...
extern "C"
{
	...
	static inline BOOL freerdp_swscale_init(void)
	{
		return TRUE;
	}

	static inline BOOL freerdp_swscale_available(void)
	{
		return TRUE;
	}

	static inline struct SwsContext* freerdp_sws_getContext(int srcW, int srcH, int srcFormat,
	                                                        int dstW, int dstH, int dstFormat,
	                                                        int flags, void* srcFilter,
	                                                        void* dstFilter, const double* param)
	{
		return sws_getContext(srcW, srcH, (enum AVPixelFormat)srcFormat, dstW, dstH,
		                      (enum AVPixelFormat)dstFormat, flags,
		                      (SwsFilter*)srcFilter, (SwsFilter*)dstFilter, param);
	}

	static inline int freerdp_sws_scale(struct SwsContext* ctx, const uint8_t* const srcSlice[],
	...
	static inline void freerdp_sws_freeContext(struct SwsContext* ctx)
	{
		sws_freeContext(ctx);
	}

	static inline BOOL freerdp_avutil_init(void)
	{
		return TRUE;
	}

	static inline BOOL freerdp_avutil_available(void)
	{
		return TRUE;
	}

	static inline int freerdp_av_image_fill_linesizes(int linesizes[4], int pix_fmt, int width)
	{
		return av_image_fill_linesizes(linesizes, (enum AVPixelFormat)pix_fmt, width);
	}

	static inline int freerdp_av_image_fill_pointers(uint8_t* data[4], int pix_fmt, int height,
	                                                 uint8_t* ptr, const int linesizes[4])
	{
		return av_image_fill_pointers(data, (enum AVPixelFormat)pix_fmt, height, ptr, linesizes);
	}
...
#endif /* FREERDP_LIB_CODEC_IMAGE_FFMPEG_H */
