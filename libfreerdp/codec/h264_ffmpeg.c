...
static BOOL libavcodec_create_encoder(H264_CONTEXT* WINPR_RESTRICT h264)
{
	...
	if (av_hwdevice_ctx_create(&sys->hwctx, AV_HWDEVICE_TYPE_VAAPI, VAAPI_DEVICE, NULL, 0) < 0)
	{
                               ...
                               BYTE** WINPR_RESTRICT ppDstData, UINT32* WINPR_RESTRICT pDstSize)
{
	...
	if ((status = av_hwframe_get_buffer(sys->codecEncoderContext->hw_frames_ctx, sys->hwVideoFrame,
	                                    0)) < 0 ||
...
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(57, 48, 101)
#ifdef WITH_VAAPI_H264_ENCODING
	status = avcodec_send_frame(sys->codecEncoderContext, sys->hwVideoFrame);
#else
