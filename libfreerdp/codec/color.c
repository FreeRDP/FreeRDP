                         ...
                         UINT32 nXSrc, UINT32 nYSrc, UINT32 nSrcWidth, UINT32 nSrcHeight)
{
	...
	{
#if defined(WITH_SWSCALE_LOADING)
		/* Check if swscale is available at runtime */
		if (!freerdp_swscale_available())
		{
			/* Gracefully degrade - no scaling support */
			WLog_WARN(TAG, "swscale not available - cannot scale image");
			return FALSE;
		}
#endif
...
#if defined(WITH_SWSCALE_LOADING)
		resize = freerdp_sws_getContext((int)nSrcWidth, (int)nSrcHeight, srcFormat,
		                                (int)nDstWidth, (int)nDstHeight, dstFormat, SWS_BILINEAR,
		                                NULL, NULL, NULL);
#else
