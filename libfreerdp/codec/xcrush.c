...
typedef struct
{
	...
	UINT32 ChunkOffset;
	UINT32 MatchLength;
} DECLSPEC_ALIGN(16) XCRUSH_MATCH_INFO;

typedef struct
{
	UINT32 offset;
	UINT32 next;
} DECLSPEC_ALIGN(8) XCRUSH_CHUNK;

typedef struct
{
	UINT16 seed;
	UINT16 size;
} DECLSPEC_ALIGN(4) XCRUSH_SIGNATURE;
...
} DECLSPEC_ALIGN(8) RDP61_MATCH_DETAILS;
...
typedef struct
{
	...
	BYTE* Literals;
} DECLSPEC_ALIGN(32) RDP61_COMPRESSED_DATA;
