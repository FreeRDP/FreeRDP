                            ...
                            size_t h264_max_size)
{
	...
	length = (uint16_t)(length - header_length - 6);
                                  ...
                                  FREERDP_VIDEO_FORMAT dstFormat, wStream* output)
{
	...
	if (dstFormat == FREERDP_VIDEO_FORMAT_H264)
	{
		...
		if (srcCompressed)
		{
			const BYTE* const_intermediate_data[4] = { intermediate_data[0], intermediate_data[1],
		...
		else
		{
			...
			const BYTE* const_srcPlanes[4] = { srcPlanes[0], srcPlanes[1], srcPlanes[2],
				                                srcPlanes[3] };
			if (!freerdp_video_convert_to_yuv(context, const_srcPlanes, srcStrides, srcFormat,
			                                  yuvData, yuvLineSizes, yuvFormat, context->width,
			                                  context->height))
			{
				WLog_ERR(TAG, "YUV conversion failed");
				return FALSE;
                                  ...
                                  FREERDP_VIDEO_FRAME* dst)
{
	...
	if (needsConversion)
	{
		const BYTE* const_intermediate_data[4] = { intermediate_data[0], intermediate_data[1],
			                                       intermediate_data[2], intermediate_data[3] };
		if (!freerdp_video_convert_to_yuv(context, const_intermediate_data, intermediate_linesize,
		                                  intermediate_format, dst->raw.data, dst->raw.linesize,
