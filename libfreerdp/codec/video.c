...
#define TAG "com.freerdp.codec.video"

#if defined(WITH_SWSCALE)

/* Include appropriate headers based on loading mode */
...
#if defined(WITH_FFMPEG) && !defined(WITH_SWSCALE_LOADING)
...
static enum AVPixelFormat video_format_to_av(FREERDP_VIDEO_FORMAT format)
{
	switch (format)
	{
		...
		case VIDEO_FORMAT_YUV411P:
			return AV_PIX_FMT_YUV411P;
		case VIDEO_FORMAT_YUVJ420P:
			return AV_PIX_FMT_YUVJ420P;
		case VIDEO_FORMAT_YUVJ422P:
			return AV_PIX_FMT_YUVJ422P;
		case VIDEO_FORMAT_YUVJ444P:
			return AV_PIX_FMT_YUVJ444P;
		case VIDEO_FORMAT_YUVJ440P:
			return AV_PIX_FMT_YUVJ440P;
		case VIDEO_FORMAT_YUV440P:
                                  ...
                                  FREERDP_VIDEO_FORMAT dstFormat, UINT32 width, UINT32 height)
{
...
#else
		sws = sws_getContext((int)width, (int)height, srcPixFmt, (int)width, (int)height,
	...
	result = freerdp_sws_scale(sws, cSrcData, srcLineSize, 0, (int)height,
	                           (uint8_t* const*)dstData, dstLineSize);
#else
	result = sws_scale(sws, cSrcData, srcLineSize, 0, (int)height, (uint8_t* const*)dstData,
	                   dstLineSize);
#endif
...
#else
	if (av_image_fill_linesizes(lineSize, pixFmt, (int)width) < 0)
#endif
	{
...
#else
	if (av_image_fill_pointers(data, pixFmt, (int)height, WINPR_CAST_CONST_PTR_AWAY(buffer, BYTE*),
	                           lineSize) < 0)
#endif
	{
