...
static BOOL update_gdi_fast_index(rdpContext* context, const FAST_INDEX_ORDER* fastIndex)
{
	...
	if (opRight >
	    (INT64)freerdp_settings_get_uint32(context->instance->settings, FreeRDP_DesktopWidth))
		opRight =
		    (int)freerdp_settings_get_uint32(context->instance->settings, FreeRDP_DesktopWidth);
...
rdpGlyphCache* glyph_cache_new(rdpSettings* settings)
{
	...
	glyphCache->context =
	    ((freerdp*)freerdp_settings_get_pointer(settings, FreeRDP_instance))->context;

	for (i = 0; i < 10; i++)
	{
		const GLYPH_CACHE_DEFINITION* cache =
		    (const GLYPH_CACHE_DEFINITION*)freerdp_settings_get_pointer_array(
		        settings, FreeRDP_GlyphCache, i);
		glyphCache->glyphCache[i].number = cache->cacheEntries;
		glyphCache->glyphCache[i].maxCellSize = cache->cacheMaximumCellSize;
