...
BOOL freerdp_capability_buffer_resize(rdpSettings* settings, size_t count)
{
	...
	{
		uint32_t* ptr = realloc(settings->ReceivedCapabilityDataSizes, count * sizeof(uint32_t));
		if (!ptr)
			return FALSE;
		settings->ReceivedCapabilityDataSizes = ptr;
		for (size_t x = len; x < count; x++)
			ptr[x] = 0;
	}
	{
		uint8_t** ptr =
		    (uint8_t**)realloc((void*)settings->ReceivedCapabilityData, count * sizeof(uint8_t*));
		if (!ptr)
			return FALSE;
		settings->ReceivedCapabilityData = ptr;
		for (size_t x = len; x < count; x++)
			ptr[x] = NULL;
	}
	{
		uint8_t* ptr = realloc(settings->ReceivedCapabilities, count * sizeof(uint8_t));
		if (!ptr)
			return FALSE;
		settings->ReceivedCapabilities = ptr;
		for (size_t x = len; x < count; x++)
			ptr[x] = 0;
	}
...
BOOL freerdp_target_net_addresses_resize(rdpSettings* settings, size_t count)
{
	...
	{
		char** ptr = (char**)realloc((void*)settings->TargetNetAddresses, count * sizeof(char*));
		if (!ptr)
			return FALSE;
		settings->TargetNetAddresses = ptr;
		for (size_t x = len; x < count; x++)
			ptr[x] = NULL;
	}
	{
		UINT32* ptr = realloc(settings->TargetNetPorts, count * sizeof(uint32_t));
		if (!ptr)
			return FALSE;
		settings->TargetNetPorts = ptr;
		for (size_t x = len; x < count; x++)
			ptr[x] = 0;
	}
                              ...
                              FreeRDP_Settings_Keys_Pointer id)
{
	...
	switch (id)
	{
		...
		case FreeRDP_RdpServerRsaKey:
		{
			WINPR_JSON* item = NULL;
			size_t len = 0;
			char* pem = freerdp_key_get_pem(val, &len);
			if (pem)
				item = WINPR_JSON_CreateString(pem);
			free(pem);
			if (!item)
                                ...
                                FreeRDP_Settings_Keys_Pointer id)
{
	...
	switch (id)
	{
		...
		case FreeRDP_RdpServerRsaKey:
		{
			...
			rdpPrivateKey* key = freerdp_key_new_from_pem(pem);
			if (!key)
				return FALSE;
