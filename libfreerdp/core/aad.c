...
/* '%s' conversions of the redirect format: authority and tenant id */
...
/* used as a winpr_asprintf() format: literal text, '%%' and at most two '%s' */
                                  ...
                                  FreeRDP_AadCloud* cloud)
{
	...
	if (_stricmp(name, aad_clouds[0].name) == 0)
	{
		WLog_WARN(TAG, "[%s] entry %" PRIuz " ignored: '%s' can not be overridden", path, index,
		          name);
		return FALSE;
	}
...
void freerdp_utils_aad_cloud_table_reset(void)
{
	...
	/* InitOnceInitialize() is a stub in WinPR's own implementation */
