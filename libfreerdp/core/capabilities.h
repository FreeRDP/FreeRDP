...
FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rdp_recv_get_active_header(rdpRdp* rdp, wStream* s,
                                                                   UINT16* pChannelId,
                                                                   UINT16* length);
FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rdp_recv_demand_active(rdpRdp* rdp, wStream* s,
                                                               UINT16 pduSource, UINT16 length);
FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rdp_send_demand_active(rdpRdp* rdp);
FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rdp_recv_confirm_active(rdpRdp* rdp, wStream* s,
                                                                UINT16 pduLength);
FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rdp_send_confirm_active(rdpRdp* rdp);

FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rdp_read_capability_set(wLog* log, wStream* sub,
                                                                UINT16 type, rdpSettings* settings,
                                                                BOOL isServer);

FREERDP_LOCAL WINPR_ATTR_NODISCARD const char* rdp_input_flag_string(UINT16 flags, char* buffer,
                                                                     size_t len);
