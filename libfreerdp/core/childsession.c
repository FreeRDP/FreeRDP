...
static int transport_bio_named_write(BIO* bio, const char* buf, int size)
{
	...
	BOOL ret = WriteFile(ptr->hFile, buf, WINPR_SAFE_INT_CAST(uint32_t, size), &written, NULL);
...
static int transport_bio_named_read(BIO* bio, char* buf, int size)
{
	...
	if ((size >= 0) && ret)
	{
		...
		const int nchunks =
		    ringbuffer_peek(&ptr->readBuffer, chunks, WINPR_SAFE_INT_CAST(size_t, ret));
		...
		ringbuffer_commit_read_bytes(&ptr->readBuffer, WINPR_SAFE_INT_CAST(size_t, ret));
