...
FREERDP_LOCAL WINPR_ATTR_NODISCARD int rdp_redirection_apply_settings(rdpRdp* rdp);

FREERDP_LOCAL WINPR_ATTR_NODISCARD state_run_t
rdp_recv_enhanced_security_redirection_packet(rdpRdp* rdp, wStream* s);
...
FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rdp_redirection_read_target_cert(
    rdpCertificate** ptargetCertificate, const BYTE* data, size_t length);

FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rdp_set_target_certificate(rdpSettings* settings,
