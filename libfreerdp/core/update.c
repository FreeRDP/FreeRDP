...
static BOOL update_write_bitmap_data(rdpUpdate* update_pub, wStream* s, BITMAP_DATA* bitmapData)
{
	...
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, bitmapData->destLeft));
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, bitmapData->destTop));
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, bitmapData->destRight));
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, bitmapData->destBottom));
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, bitmapData->width));
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, bitmapData->height));
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, bitmapData->bitsPerPixel));
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, bitmapData->flags));
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, bitmapData->bitmapLength));
	...
	if (bitmapData->flags & BITMAP_COMPRESSION)
	{
		if (!(bitmapData->flags & NO_BITMAP_COMPRESSION_HDR))
		{
			...
			Stream_Write_UINT16(
			    s,
			    WINPR_SAFE_INT_CAST(uint16_t, bitmapData->cbScanWidth)); /* cbScanWidth (2 bytes) */
			Stream_Write_UINT16(
			    s,
			    WINPR_SAFE_INT_CAST(
                                       ...
                                       const BITMAP_UPDATE* bitmapUpdate)
{
	...
	Stream_Write_UINT16(
	    s, WINPR_SAFE_INT_CAST(uint16_t, bitmapUpdate->number)); /* numberRectangles (2 bytes) */
                                   ...
                                   size_t offset)
{
	...
	if (orderInfo->controlFlags & ORDER_TYPE_CHANGE)
		Stream_Write_UINT8(
		    s, WINPR_SAFE_INT_CAST(uint8_t, orderInfo->orderType)); /* orderType (1 byte) */
                                         ...
                                         const POINTER_POSITION_UPDATE* pointerPosition)
{
	...
	Stream_Write_UINT16(s,
	                    WINPR_SAFE_INT_CAST(uint16_t, pointerPosition->xPos)); /* xPos (2 bytes) */
	Stream_Write_UINT16(s,
	                    WINPR_SAFE_INT_CAST(uint16_t, pointerPosition->yPos)); /* yPos (2 bytes) */
...
static BOOL update_send_pointer_new(rdpContext* context, const POINTER_NEW_UPDATE* pointer_new)
{
	...
	Stream_Write_UINT16(s,
	                    WINPR_SAFE_INT_CAST(uint16_t, pointer_new->xorBpp)); /* xorBpp (2 bytes) */
                                       ...
                                       const POINTER_CACHED_UPDATE* pointer_cached)
{
	...
	Stream_Write_UINT16(
	    s, WINPR_SAFE_INT_CAST(uint16_t, pointer_cached->cacheIndex)); /* cacheIndex (2 bytes) */
                                               ...
                                               const WINDOW_STATE_ORDER* stateOrder)
{
	...
	if ((orderInfo->fieldFlags & WINDOW_ORDER_FIELD_SHOW) != 0)
	{
		Stream_Write_UINT8(s, WINPR_SAFE_INT_CAST(uint8_t, stateOrder->showState));
	}
	...
	if ((orderInfo->fieldFlags & WINDOW_ORDER_FIELD_RP_CONTENT) != 0)
	{
		Stream_Write_UINT8(s, WINPR_SAFE_INT_CAST(uint8_t, stateOrder->RPContent));
	}
	...
	if ((orderInfo->fieldFlags & WINDOW_ORDER_FIELD_WND_RECTS) != 0)
	{
		Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, stateOrder->numWindowRects));
	...
	if ((orderInfo->fieldFlags & WINDOW_ORDER_FIELD_VISIBILITY) != 0)
	{
		Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, stateOrder->numVisibilityRects));
                                    ...
                                    const WINDOW_ICON_ORDER* iconOrder)
{
	...
	/* Write body */
	Stream_Write_UINT16(
	    s, WINPR_SAFE_INT_CAST(uint16_t, iconInfo->cacheEntry)); /* CacheEntry (2 bytes) */
	Stream_Write_UINT8(s, WINPR_SAFE_INT_CAST(uint8_t, iconInfo->cacheId));  /* CacheId (1 byte) */
	Stream_Write_UINT8(s, WINPR_SAFE_INT_CAST(uint8_t, iconInfo->bpp));      /* Bpp (1 byte) */
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, iconInfo->width));  /* Width (2 bytes) */
	Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, iconInfo->height)); /* Height (2 bytes) */

	if (iconInfo->bpp <= 8)
	{
		Stream_Write_UINT16(
		    s, WINPR_SAFE_INT_CAST(uint16_t, iconInfo->cbColorTable)); /* CbColorTable (2 bytes) */
	}

	Stream_Write_UINT16(
	    s, WINPR_SAFE_INT_CAST(uint16_t, iconInfo->cbBitsMask)); /* CbBitsMask (2 bytes) */
	Stream_Write_UINT16(
	    s, WINPR_SAFE_INT_CAST(uint16_t, iconInfo->cbBitsColor)); /* CbBitsColor (2 bytes) */
                                           ...
                                           const WINDOW_CACHED_ICON_ORDER* cachedIconOrder)
{
	...
	/* Write body */
	Stream_Write_UINT16(
	    s, WINPR_SAFE_INT_CAST(uint16_t, cachedIcon->cacheEntry)); /* CacheEntry (2 bytes) */
	Stream_Write_UINT8(s, WINPR_SAFE_INT_CAST(uint8_t, cachedIcon->cacheId)); /* CacheId (1 byte) */
	update->numberOrders++;
	return TRUE;
}
                                               ...
                                               const NOTIFY_ICON_STATE_ORDER* iconStateOrder)
{
	...
	if ((orderInfo->fieldFlags & WINDOW_ORDER_ICON) != 0)
	{
		...
		Stream_Write_UINT8(s,
		                   WINPR_SAFE_INT_CAST(uint8_t, iconInfo.cacheId)); /* CacheId (1 byte) */
		Stream_Write_UINT8(s, WINPR_SAFE_INT_CAST(uint8_t, iconInfo.bpp));  /* Bpp (1 byte) */
		Stream_Write_UINT16(s, WINPR_SAFE_INT_CAST(uint16_t, iconInfo.width)); /* Width (2 bytes) */
		Stream_Write_UINT16(s,
		                    WINPR_SAFE_INT_CAST(uint16_t, iconInfo.height)); /* Height (2 bytes) */

		if (iconInfo.bpp <= 8)
		{
			Stream_Write_UINT16(
			    s,
			    WINPR_SAFE_INT_CAST(uint16_t, iconInfo.cbColorTable)); /* CbColorTable (2 bytes) */
		}
	...
	else if ((orderInfo->fieldFlags & WINDOW_ORDER_CACHED_ICON) != 0)
	{
		...
		Stream_Write_UINT16(
		    s, WINPR_SAFE_INT_CAST(uint16_t, cachedIcon.cacheEntry)); /* CacheEntry (2 bytes) */
		Stream_Write_UINT8(s,
		                   WINPR_SAFE_INT_CAST(uint8_t, cachedIcon.cacheId)); /* CacheId (1 byte) */
	}
	...
	if (orderInfo->fieldFlags & WINDOW_ORDER_FIELD_DESKTOP_ZORDER)
	{
		Stream_Write_UINT8(
		    s, WINPR_SAFE_INT_CAST(uint8_t,
		                           monitoredDesktop->numWindowIds)); /* numWindowIds (1 byte) */

		/* windowIds */
		for (UINT32 i = 0; i < monitoredDesktop->numWindowIds; i++)
		{
			Stream_Write_UINT32(s, WINPR_SAFE_INT_CAST(uint32_t, monitoredDesktop->windowIds[i]));
		}
	}
