...
struct _KRB5FunctionTable
{
	...
	krb5_error_code (*pKrb5_parse_name_flags)(krb5_context m_context, const char* m_address, int m_flags, krb5_principal* m_principal);
	krb5_error_code (*pKrb5_cc_cache_match)(krb5_context m_context, krb5_principal m_client, krb5_ccache * m_id);
	krb5_error_code (*pKrb5_cc_default)(krb5_context m_context, krb5_ccache * m_id);
	krb5_error_code (*pKrb5_init_creds_init)(krb5_context m_context, krb5_principal m_client, krb5_prompter_fct m_prompter, void * m_prompter_data, krb5_deltat m_start_time, krb5_get_init_creds_opt * m_options, krb5_init_creds_context * m_ctx);
	krb5_error_code (*pKrb5_init_creds_set_password)(krb5_context m_context, krb5_init_creds_context m_ctx, const char * m_password);
...
UINT32 transport_krb5_check_account(char* username, char* domain, char* passwd)
{
	...
	g_KRB5.pKrb5_init_creds_set_password 	= (void*) GetProcAddress(g_KRB5Module, "krb5_init_creds_set_password");
	...
	if (where | SSL_CB_ALERT) 
	{
		if (ret == 561) {
			...
			if (transport->NlaMode) {
				UINT32 kret = 0;
#ifdef WITH_KRB5_SUPPORT
				if ((strlen(transport->settings->Domain) != 0) && (strncmp(transport->settings->Domain, ".", 1) != 0)) {
					
					kret = transport_krb5_check_account(transport->settings->Username, transport->settings->Domain, transport->settings->Password);					
