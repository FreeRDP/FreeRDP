...
FREERDP_LOCAL WINPR_ATTR_NODISCARD websocket_context* websocket_context_new(void);

FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL websocket_context_reset(websocket_context* context);

FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL websocket_context_write_wstream(websocket_context* context,
                                                                        BIO* bio, wStream* sPacket,
                                                                        WEBSOCKET_OPCODE opcode);
FREERDP_LOCAL WINPR_ATTR_NODISCARD int websocket_context_write(websocket_context* context, BIO* bio,
                                                               const BYTE* buf, int isize,
                                                               WEBSOCKET_OPCODE opcode);
FREERDP_LOCAL WINPR_ATTR_NODISCARD int websocket_context_read(websocket_context* encodingContext,
                                                              BIO* bio, BYTE* pBuffer, size_t size);

WINPR_ATTR_MALLOC(Stream_Free, 1)
FREERDP_LOCAL WINPR_ATTR_NODISCARD wStream*
websocket_context_packet_new(size_t len, WEBSOCKET_OPCODE opcode, UINT32* pMaskingKey);

FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL websocket_context_mask_and_send(BIO* bio, wStream* sPacket,
                                                                        wStream* sDataPacket,
                                                                        UINT32 maskingKey);
