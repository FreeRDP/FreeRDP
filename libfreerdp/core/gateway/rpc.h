...
FREERDP_LOCAL WINPR_ATTR_NODISCARD const char* rpc_vc_state_str(VIRTUAL_CONNECTION_STATE state);
FREERDP_LOCAL void rpc_pdu_header_print(wLog* log, const rpcconn_hdr_t* header);
FREERDP_LOCAL WINPR_ATTR_NODISCARD rpcconn_common_hdr_t rpc_pdu_header_init(const rdpRpc* rpc);

FREERDP_LOCAL WINPR_ATTR_NODISCARD size_t rpc_offset_align(size_t* offset, size_t alignment);
FREERDP_LOCAL WINPR_ATTR_NODISCARD size_t rpc_offset_pad(size_t* offset, size_t pad);

FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rpc_get_stub_data_info(rdpRpc* rpc,
...
FREERDP_LOCAL WINPR_ATTR_NODISCARD SSIZE_T rpc_channel_write_int(RpcChannel* channel,
                                                                 const BYTE* data, size_t length,
                                                                 const char* file, size_t line,
                                                                 const char* fkt);

FREERDP_LOCAL WINPR_ATTR_NODISCARD SSIZE_T rpc_channel_read(RpcChannel* channel, wStream* s,
                                                            size_t length);

FREERDP_LOCAL void rpc_channel_free(RpcChannel* channel);

WINPR_ATTR_MALLOC(rpc_channel_free, 1)
FREERDP_LOCAL WINPR_ATTR_NODISCARD RpcOutChannel* rpc_out_channel_new(rdpRpc* rpc,
                                                                      const GUID* guid);
FREERDP_LOCAL WINPR_ATTR_NODISCARD int
rpc_out_channel_replacement_connect(RpcOutChannel* outChannel, uint32_t timeout);

FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL
rpc_in_channel_transition_to_state(RpcInChannel* inChannel, CLIENT_IN_CHANNEL_STATE state);
FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL
rpc_out_channel_transition_to_state(RpcOutChannel* outChannel, CLIENT_OUT_CHANNEL_STATE state);

FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rpc_virtual_connection_transition_to_state(
    rdpRpc* rpc, RpcVirtualConnection* connection, VIRTUAL_CONNECTION_STATE state);

FREERDP_LOCAL WINPR_ATTR_NODISCARD BOOL rpc_connect(rdpRpc* rpc, UINT32 timeout);

FREERDP_LOCAL void rpc_free(rdpRpc* rpc);

WINPR_ATTR_MALLOC(rpc_free, 1)
FREERDP_LOCAL WINPR_ATTR_NODISCARD rdpRpc* rpc_new(rdpTransport* transport);
