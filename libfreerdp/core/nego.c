...
BOOL nego_connect(rdpNego* nego)
{
	...
	if (!freerdp_settings_set_uint32(settings, FreeRDP_RequestedProtocols,
	    ...
	    !freerdp_settings_set_uint32(settings, FreeRDP_SelectedProtocol, nego->SelectedProtocol) ||
...
BOOL nego_send_negotiation_response(rdpNego* nego)
{
	...
	if (status)
	{
		...
		if (freerdp_settings_get_uint32(settings, FreeRDP_SelectedProtocol) == PROTOCOL_RDP)
		{
			...
			if (!freerdp_settings_get_pointer(settings, FreeRDP_RdpServerRsaKey) &&
			    !freerdp_settings_get_string(settings, FreeRDP_RdpKeyFile) &&
			    !freerdp_settings_get_string(settings, FreeRDP_RdpKeyContent))
			{
				WLog_ERR(TAG, "Missing server certificate");
				return FALSE;
			}
		}
		else if (freerdp_settings_get_uint32(settings, FreeRDP_SelectedProtocol) == PROTOCOL_SSL)
		{
		...
		else if (freerdp_settings_get_uint32(settings, FreeRDP_SelectedProtocol) == PROTOCOL_HYBRID)
		{
			if (!freerdp_settings_set_bool(settings, FreeRDP_TlsSecurity, TRUE) ||
