...
static void rdp_write_synchronize_pdu(wStream* s, rdpSettings* settings)
{
	...
	Stream_Write_UINT16(
	    s, freerdp_settings_get_uint32(settings, FreeRDP_PduSource)); /* targetUser (2 bytes) */
}
...
BOOL rdp_send_deactivate_all(rdpRdp* rdp)
{
	...
	Stream_Write_UINT32(
	    s, freerdp_settings_get_uint32(rdp->settings, FreeRDP_ShareId)); /* shareId (4 bytes) */
...
BOOL rdp_server_accept_client_font_list_pdu(rdpRdp* rdp, wStream* s)
{
	...
	if (freerdp_settings_get_bool(settings, FreeRDP_SupportMonitorLayoutPdu) &&
	    freerdp_settings_get_uint32(settings, FreeRDP_MonitorCount) && peer->AdjustMonitorsLayout &&
	    peer->AdjustMonitorsLayout(peer))
	{
		const rdpMonitor* cur =
		    (const rdpMonitor*)freerdp_settings_get_pointer(settings, FreeRDP_MonitorDefArray);
		/* client supports the monitorLayout PDU, let's send him the monitors if any */
		MONITOR_DEF* monitors = (MONITOR_DEF*)calloc(
		    freerdp_settings_get_uint32(settings, FreeRDP_MonitorCount), sizeof(MONITOR_DEF));

		if (!monitors)
			return FALSE;

		if (!display_convert_rdp_monitor_to_monitor_def(
		        freerdp_settings_get_uint32(settings, FreeRDP_MonitorCount), cur, &monitors))
		{
			free(monitors);
			return FALSE;
		}

		if (!freerdp_display_send_monitor_layout(
		        rdp->context, freerdp_settings_get_uint32(settings, FreeRDP_MonitorCount),
		        monitors))
		{
			free(monitors);
			return FALSE;
