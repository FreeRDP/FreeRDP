		...
		HANDLE *handles, DWORD *count)
{
	...
	if (!message || !count)
	{
		DEBUG_WARN("message=%p, count=%p", message, count);
		return NULL;
	}

	hdl = realloc(handles, (*count) * sizeof(HANDLE));

	if (!hdl)
	{
		DEBUG_WARN("realloc failed %s (%d)", strerror(errno), errno);
		return NULL;
	}

	hdl[(*count)++] = message->thread;
	return hdl;
}
