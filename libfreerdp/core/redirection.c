...
int rdp_redirection_apply_settings(rdpRdp* rdp)
{
	...
	if (freerdp_settings_get_uint32(settings, FreeRDP_RedirectionFlags) & LB_PASSWORD)
	{
	...
	if (freerdp_settings_get_uint32(settings, FreeRDP_RedirectionFlags) & LB_CLIENT_TSV_URL)
	{
	...
	if (freerdp_settings_get_uint32(settings, FreeRDP_RedirectionFlags) & LB_TARGET_NET_ADDRESSES)
	{
		UINT32 i;
		if (!freerdp_settings_set_pointer_len(settings, FreeRDP_TargetNetAddresses, NULL,
		                                      redirection->TargetNetAddressesCount * sizeof(char*)))
			return -1;
		...
		for (i = 0; i < freerdp_settings_get_uint32(settings, FreeRDP_TargetNetAddressCount); i++)
		{
			...
			{
				freerdp_settings_set_pointer_len(settings, FreeRDP_TargetNetAddresses, NULL, 0);
				return -1;
			}
		}
	}
