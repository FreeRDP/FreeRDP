...
int credssp_client_authenticate(rdpCredssp* credssp)
{
	...
	if (status != SEC_E_OK)
	{
		error_report("Could not verify public key echo!\n");
		return -1;
	}
	...
	if (status != SEC_E_OK)
	{
		error_report("credssp_encrypt_ts_credentials status: 0x%08X\n", status);
		return 0;
	}
...
int credssp_server_authenticate(rdpCredssp* credssp)
{
	...
	while (true)
	{
		...
		{
			error_report("CredSSP: invalid negoToken!\n");
			return -1;
		}
		...
		if (status == SEC_E_OK)
		{
			...
			{
				error_report("Error: could not verify client's public key echo\n");
				return -1;
			}
		...
		{
			error_report("AcceptSecurityContext status: 0x%08X\n", status);
			return -1;
		}
	...
	{
		error_report("Could not decrypt TSCredentials status: 0x%08X\n", status);
		return 0;
	}
	...
	if (status != SEC_E_OK)
	{
		error_report("ImpersonateSecurityContext status: 0x%08X\n", status);
		return 0;
	}
	else
	{
		...
		if (status != SEC_E_OK)
		{
			error_report("RevertSecurityContext status: 0x%08X\n", status);
			return 0;
		}
	}
...
SECURITY_STATUS credssp_decrypt_public_key_echo(rdpCredssp* credssp)
{
	...
	{
		error_report("unexpected pubKeyAuth buffer size:%d\n", (int) credssp->pubKeyAuth.cbBuffer);
	...
	if (status != SEC_E_OK)
	{
		error_report("DecryptMessage failure: 0x%08X\n", status);
		return status;
	}
	...
	{
		error_report("Could not verify server's public key echo\n");
...
SECURITY_STATUS credssp_decrypt_ts_credentials(rdpCredssp* credssp)
{
	...
	{
		error_report("credssp_decrypt_ts_credentials missing authInfo buffer\n");
		return SEC_E_INVALID_TOKEN;
	}
...
int credssp_recv(rdpCredssp* credssp)
{
	...
	if (status < 0)
	{
		error_report("credssp_recv() error: %d\n", status);
		stream_free(s);
		return -1;
	}
