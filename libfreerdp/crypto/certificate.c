...
static BOOL certificate_read_x509_certificate(const rdpCertBlob* cert, rdpCertInfo* info)
{
	...
	/* signature */
	if (!ber_read_sequence_tag(s, &length) || !Stream_SafeSeek(s, length))
	{ /* AlgorithmIdentifier (SEQUENCE) */
		goto error;
	}
	...
	/* subjectPublicKeyInfo::subjectPublicKey */
	if (!ber_read_bit_string(s, &length, &padding))
	{ /* BIT_STRING */
		goto error;
	}
	...
	if (!ber_read_integer_length(s, &modulus_length))
	{ /* modulus (INTEGER) */
		goto error;
	}
	...
	if (!ber_read_integer_length(s, &exponent_length))
	{ /* publicExponent (INTEGER) */
		goto error;
	}
...
static BOOL certificate_read_server_proprietary_certificate(rdpCertificate* certificate, wStream* s)
{
	...
	if (!certificate_process_server_public_key(certificate, s))
	{
		return FALSE;
	}
