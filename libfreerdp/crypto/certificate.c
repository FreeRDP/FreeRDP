...
static BOOL certificate_read_server_proprietary_certificate(rdpCertificate* certificate, wStream* s)
{
	...
	sigdatalen = WINPR_SAFE_INT_CAST(size_t, Stream_PointerAs(s, const BYTE) - sigdata);
                                        ...
                                        DWORD* PublicKeyLength)
{
	...
	*PublicKey = optr = ptr = (BYTE*)calloc(WINPR_SAFE_INT_CAST(size_t, length), sizeof(BYTE));
                                           ...
                                           size_t cbInput, BYTE** poutput, size_t* pcbOutput)
{
	...
	size_t outputSize = WINPR_SAFE_INT_CAST(size_t, EVP_PKEY_size(pkey));
...
BYTE* freerdp_certificate_get_der(const rdpCertificate* cert, size_t* pLength)
{
	...
	BYTE* ptr = calloc(WINPR_SAFE_INT_CAST(size_t, rc) + 1, sizeof(BYTE));
	if (!ptr)
		return NULL;
	BYTE* i2d_ptr = ptr;
                                    ...
                                    size_t* psize)
{
	...
	*psize = 0;
	...
	const size_t bnsize = WINPR_SAFE_INT_CAST(size_t, BN_num_bytes(bn));
